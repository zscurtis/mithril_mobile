(function () {
  ...
  var RELEASE_VERSION = "m39.3";
  var RELEASE_LABEL = "depth ranges and hole diameter on both summaries, plus Shot Diagram explosive-load extremes, selection and transfer tools, structured hole conditions, GPS callouts, multi quick fill, and canvas themes";
  var THEME_STORAGE_KEY = "mithrilCanvasThemeV1";
  var THEME_CLASS_PREFIX = "m393-theme-";
  var THEME_OPTIONS = [
  ...
  function installPrecisionCanvasCoordinates(canvas, type) {
    if (!canvas || canvas.getAttribute("data-m393-precision-coordinates") === "true") return;
    ...
    canvas.setAttribute("data-m393-precision-coordinates", "true");
  }

  function enableWheelZoom(canvas) {
    if (!canvas || canvas.getAttribute("data-m393-wheel-zoom") === "true") return;
    canvas.setAttribute("data-m393-wheel-zoom", "true");
  ...
  function addShotInfoBackButton() {
    ...
    if (!modal || byId("m393ShotInfoBack")) return;
    var grid = modal.querySelector(".buttonGrid");
    if (!grid) return;

    var button = document.createElement("button");
    button.id = "m393ShotInfoBack";
    button.type = "button";
    button.className = "m393BackMenu";
  ...
  function installShotFrameBridge() {
    ...
    if (!frame || frame.getAttribute("data-m393-bridge") === "true") return;
    frame.setAttribute("data-m393-bridge", "true");

    function injectChildScript() {
      try {
        var childDocument = frame.contentDocument;
        if (!childDocument || !childDocument.documentElement) return false;
        if (childDocument.getElementById("mithrilMenuM393ChildLoader")) return true;

        var script = childDocument.createElement("script");
        script.id = "mithrilMenuM393ChildLoader";
        script.src = "./mithril-menu-m393.js?v=39.3-frame";
  ...
  function injectStyles() {
    if (byId("mithrilMenuM393Styles")) return;

    var style = document.createElement("style");
    style.id = "mithrilMenuM393Styles";
    style.textContent = [
      ".m393MenuIntro{margin:0 0 10px;color:#4b4b4b;font-size:13px;font-weight:750;line-height:1.35}",
      ".m393MenuStack{display:grid;grid-template-columns:1fr;gap:8px}",
      ".m393MenuStack>button{width:100%;min-height:52px;text-align:left;padding:10px 13px;font-size:16px}",
      ".m393MenuStack>button.m393Home{text-align:center}",
      ".m393Section{display:none;margin-top:9px;padding:10px;border:1px solid #bcbcbc;border-radius:11px;background:#f8f8f8}",
      ".m393Section.show{display:block}",
      ".m393SectionTitle{margin:0 0 8px;font-size:16px;font-weight:950}",
      ".m393SectionHelp{margin:0 0 9px;color:#555;font-size:12px;font-weight:750;line-height:1.35}",
      ".m393ActionGrid{display:grid;grid-template-columns:1fr 1fr;gap:8px}",
      ".m393ActionGrid button{min-height:49px}",
      ".m393ActionGrid .wide{grid-column:1/-1}",
      ".m393Subpanel{display:none;grid-column:1/-1;padding:9px;border:1px solid #c7c7c7;border-radius:10px;background:white}",
      ".m393Subpanel.show{display:block}",
      ".m393DirectionGrid{display:grid;grid-template-columns:1fr 1fr 1fr;grid-template-areas:'. up .' 'left center right' '. down .';gap:8px}",
      ".m393DirectionGrid button{min-height:50px;padding:7px 5px}",
      ".m393Up{grid-area:up}.m393Left{grid-area:left}.m393Center{grid-area:center}.m393Right{grid-area:right}.m393Down{grid-area:down}",
      ".m393Spacer{visibility:hidden;pointer-events:none}",
      ".m393DangerZone{margin-top:10px;padding-top:10px;border-top:1px solid #d6aaaa}",
      ".m393DangerZone button{width:100%;min-height:50px}",
      ".m393BackMenu{grid-column:1/-1;background:#eef4ff;border-color:#7aa2d8}",
      ".m393ThemePanel{max-height:46vh;overflow:auto;padding-right:2px}",
      ".m393ThemeGroupTitle{margin:10px 0 6px;font-size:12px;font-weight:950;color:#555;text-transform:uppercase;letter-spacing:.04em}",
      ".m393ThemeGrid{display:grid;grid-template-columns:1fr 1fr;gap:8px}",
      ".m393ThemeButton{min-height:44px;font-size:13px;line-height:1.25;text-align:left}",
      ".m393ThemeButton.active{background:#1f6feb;color:#fff;border-color:#1f6feb}",
      "html.m393-theme-gray,body.m393-theme-gray{background:#2e2e2e !important}",
      "html.m393-theme-dark-slate,body.m393-theme-dark-slate{background-color:#232a31 !important;background-image:radial-gradient(circle at 18% 18%, rgba(255,255,255,.06) 0 3px, transparent 4px),radial-gradient(circle at 76% 70%, rgba(0,0,0,.2) 0 18px, transparent 20px),linear-gradient(135deg,#20262d 0%,#313b46 100%) !important;background-size:140px 140px,220px 220px,cover !important;background-attachment:fixed !important}",
      "html.m393-theme-blue-steel,body.m393-theme-blue-steel{background-color:#566575 !important;background-image:radial-gradient(circle at 20% 20%, rgba(255,255,255,.08) 0 2px, transparent 3px),linear-gradient(135deg,rgba(255,255,255,.08),rgba(255,255,255,0) 38%),linear-gradient(135deg,#4b5a68 0%,#6b7d8f 100%) !important;background-size:150px 150px,cover,cover !important;background-attachment:fixed !important}",
      "html.m393-theme-subtle-grid,body.m393-theme-subtle-grid{background-color:#252e38 !important;background-image:radial-gradient(circle at center, rgba(255,255,255,.03) 1px, transparent 1px),repeating-linear-gradient(0deg, rgba(255,255,255,.06) 0 1px, transparent 1px 26px),repeating-linear-gradient(90deg, rgba(255,255,255,.06) 0 1px, transparent 1px 26px),linear-gradient(135deg,#232b34,#2e3945) !important;background-size:26px 26px,26px 26px,26px 26px,cover !important;background-attachment:fixed !important}",
      "html.m393-theme-gradient-slate,body.m393-theme-gradient-slate{background:#54606f !important;background-image:linear-gradient(135deg,#6b7786 0%,#3c4653 100%) !important;background-attachment:fixed !important}",
      "html.m393-theme-dark-paper,body.m393-theme-dark-paper{background-color:#35383d !important;background-image:radial-gradient(circle at 25% 25%, rgba(255,255,255,.05) 0 2px, transparent 3px),radial-gradient(circle at 75% 60%, rgba(255,255,255,.03) 0 1px, transparent 2px),linear-gradient(135deg,#2c3035 0%,#44484f 100%) !important;background-size:120px 120px,90px 90px,cover !important;background-attachment:fixed !important}",
      "html.m393-theme-soft-quarry-tan,body.m393-theme-soft-quarry-tan{background-color:#b9aea0 !important;background-image:radial-gradient(circle at 20% 20%, rgba(255,255,255,.12) 0 2px, transparent 3px),radial-gradient(circle at 80% 70%, rgba(0,0,0,.08) 0 2px, transparent 3px),linear-gradient(135deg,#c8bdae 0%,#a89b8b 100%) !important;background-size:70px 70px,90px 90px,cover !important;background-attachment:fixed !important}",
      "html.m393-theme-blast-ember,body.m393-theme-blast-ember{background-color:#111 !important;background-image:radial-gradient(circle at 15% 78%, rgba(255,110,0,.72) 0 2%, transparent 8%),radial-gradient(circle at 82% 22%, rgba(255,90,0,.48) 0 1.4%, transparent 7%),repeating-linear-gradient(135deg, rgba(255,120,0,.0) 0 18px, rgba(255,110,0,.18) 18px 19px, transparent 19px 34px),linear-gradient(135deg,#090909,#262626) !important;background-attachment:fixed !important}",
      "html.m393-theme-electric-steel,body.m393-theme-electric-steel{background-color:#0e2032 !important;background-image:radial-gradient(circle at 50% 60%, rgba(0,150,255,.34) 0 18%, transparent 42%),repeating-linear-gradient(135deg, rgba(255,255,255,.05) 0 2px, transparent 2px 18px),linear-gradient(135deg,#091521,#27425f) !important;background-attachment:fixed !important}",
      "html.m393-theme-blast-placard,body.m393-theme-blast-placard{background-color:#111 !important;background-image:linear-gradient(45deg, transparent 38%, rgba(255,150,30,.78) 38% 62%, transparent 62%),linear-gradient(-45deg, transparent 38%, rgba(255,150,30,.78) 38% 62%, transparent 62%),radial-gradient(circle at 80% 25%, rgba(255,165,0,.22) 0 12%, transparent 26%),repeating-linear-gradient(135deg, rgba(255,150,30,.22) 0 12px, transparent 12px 36px),linear-gradient(135deg,#090909,#1b1b1b) !important;background-size:280px 280px,280px 280px,cover,cover,cover !important;background-attachment:fixed !important}",
      "html.m393-theme-copper-quarry,body.m393-theme-copper-quarry{background-color:#5a2b11 !important;background-image:radial-gradient(circle at 25% 30%, rgba(255,170,100,.28) 0 14%, transparent 26%),radial-gradient(circle at 72% 68%, rgba(255,210,130,.16) 0 10%, transparent 24%),repeating-linear-gradient(45deg, rgba(255,255,255,.02) 0 12px, rgba(0,0,0,.08) 12px 22px),linear-gradient(135deg,#4c200c,#8a481f) !important;background-attachment:fixed !important}",
      "html.m393-theme-cobalt-topo,body.m393-theme-cobalt-topo{background-color:#041c3a !important;background-image:radial-gradient(circle at 18% 72%, transparent 0 40px, rgba(0,180,255,.24) 41px 42px, transparent 43px 54px, rgba(0,180,255,.18) 55px 56px, transparent 57px),radial-gradient(circle at 78% 62%, transparent 0 50px, rgba(0,180,255,.22) 51px 52px, transparent 53px 66px, rgba(0,180,255,.18) 67px 68px, transparent 69px),radial-gradient(circle at 56% 22%, transparent 0 32px, rgba(0,180,255,.18) 33px 34px, transparent 35px 48px, rgba(0,180,255,.15) 49px 50px, transparent 51px),linear-gradient(135deg,#031325,#09447a) !important;background-attachment:fixed !important}",
      "html.m393-theme-signal-red-slate,body.m393-theme-signal-red-slate{background-color:#120b0b !important;background-image:radial-gradient(circle at 18% 78%, rgba(255,70,40,.34) 0 18%, transparent 30%),radial-gradient(circle at 84% 20%, rgba(255,60,30,.28) 0 12%, transparent 24%),repeating-linear-gradient(90deg, rgba(255,60,30,.16) 0 2px, transparent 2px 48px),linear-gradient(135deg,#0b0b0b,#272222) !important;background-attachment:fixed !important}",
      "@media(max-width:520px){.m393QuickButton{font-size:0}.m393QuickButton:after{content:'Quick';font-size:14px}.m393FitButton{font-size:0}.m393FitButton:after{content:'Fit';font-size:14px}.m393ActionGrid{grid-template-columns:1fr}.m393ActionGrid .wide{grid-column:auto}.m393DirectionGrid{grid-template-columns:1fr 1fr 1fr}.m393ThemeGrid{grid-template-columns:1fr}}"
    ].join("");
  ...
  function updateToolbar(isShot) {
    ...
    for (var i = 0; i < topButtons.length; i += 1) {
      ...
      if (label === "Quick") {
        ...
        topButtons[i].classList.add("m393QuickButton");
      }
    }
    ...
    if (zoomButtons.length && String(zoomButtons[0].textContent || "").trim() === "Fit") {
      ...
      zoomButtons[0].classList.add("m393FitButton");
    }
  ...
  function hideAllSections(box, exceptId) {
    var sections = box.querySelectorAll(".m393Section");
    var buttons = box.querySelectorAll("[data-m393-section]");
    var i;
    ...
    for (i = 0; i < buttons.length; i += 1) {
      var target = buttons[i].getAttribute("data-m393-section");
      if (target !== exceptId) setButtonArrow(buttons[i], false);
    }
  }

  function hideSubpanels(section, exceptId) {
    var panels = section.querySelectorAll(".m393Subpanel");
    var buttons = section.querySelectorAll("[data-m393-subpanel]");
    var i;
    ...
    for (i = 0; i < buttons.length; i += 1) {
      var target = buttons[i].getAttribute("data-m393-subpanel");
      if (target !== exceptId) setButtonArrow(buttons[i], false);
    }
  }
  ...
  function wireExpandableSections(box) {
    box.addEventListener("click", function (event) {
      var sectionButton = event.target.closest("[data-m393-section]");
      if (sectionButton && box.contains(sectionButton)) {
        event.preventDefault();
        var sectionId = sectionButton.getAttribute("data-m393-section");
      ...
      var subButton = event.target.closest("[data-m393-subpanel]");
      if (subButton && box.contains(subButton)) {
        event.preventDefault();
        var subId = subButton.getAttribute("data-m393-subpanel");
        var subpanel = byId(subId);
        if (!subpanel) return;
        var parentSection = subButton.closest(".m393Section");
  ...
  function resetMenuPanelsWhenClosed(menu) {
    var observer = new MutationObserver(function () {
      ...
      var sections = menu.querySelectorAll(".m393Section,.m393Subpanel");
      var buttons = menu.querySelectorAll("[data-m393-section],[data-m393-subpanel]");
      var i;
  ...
  function refreshThemeButtons(root) {
    ...
    var buttons = root.querySelectorAll ? root.querySelectorAll('[data-m393-theme-choice]') : [];
    for (var i = 0; i < buttons.length; i += 1) {
      var button = buttons[i];
      var label = button.getAttribute('data-label') || button.textContent.replace(/^✓\s*/, '').trim();
      button.setAttribute('data-label', label);
      var active = button.getAttribute('data-m393-theme-choice') === current;
  ...
  function buildThemePickerHtml(panelId) {
    function buildThemeButtons(groupKey) {
      ...
      for (var i = 0; i < THEME_OPTIONS.length; i += 1) {
        ...
        html += '<button type="button" class="m393ThemeButton" data-m393-theme-choice="' + THEME_OPTIONS[i].key + '" data-label="' + THEME_OPTIONS[i].label + '">' + THEME_OPTIONS[i].label + '</button>';
      }
      return html;
    }

    return [
      '<button type="button" class="wide" data-m393-subpanel="' + panelId + '" data-label="Canvas Background" aria-expanded="false">Canvas Background  ›</button>',
      '<div id="' + panelId + '" class="m393Subpanel">',
      '  <div class="m393ThemePanel">',
      '    <p class="m393SectionHelp">Pick a canvas background. The theme applies immediately and stays saved on this device.</p>',
      '    <div class="m393ThemeGroupTitle">Classic Themes</div>',
      '    <div class="m393ThemeGrid">' + buildThemeButtons('classic') + '</div>',
      '    <div class="m393ThemeGroupTitle">Bold Themes</div>',
      '    <div class="m393ThemeGrid">' + buildThemeButtons('bold') + '</div>',
      '    <div class="m393ThemeGroupTitle">Default</div>',
      '    <div class="m393ThemeGrid">' + buildThemeButtons('reset') + '</div>',
      '  </div>',
      '</div>'
    ].join('');
  }
  ...
  function wireThemeButtons(root) {
    ...
    for (var i = 0; i < buttons.length; i += 1) {
      buttons[i].addEventListener('click', function () {
        chooseTheme(this.getAttribute('data-m393-theme-choice'));
      });
    }
  ...
  function patchDrillMenu() {
    var menu = byId("menuModal");
    if (!menu || menu.getAttribute("data-m393-patched") === "drill") return;
    var box = menu.querySelector(".box");
    if (!box) return;

    menu.setAttribute("data-m393-patched", "drill");
    box.innerHTML = [
      '<div class="boxHead"><span>Drill Log Menu</span><button type="button" data-m393-action="close">Close</button></div>',
      '<p class="m393MenuIntro">Daily tools stay visible. Setup, exports, and recovery tools open only when needed.</p>',
      '<div class="m393MenuStack">',
      '  <button type="button" data-m393-action="info">Drill Log Info</button>',
      '  <button type="button" class="primary" data-m393-action="editHoles">Edit Holes</button>',
      '  <button type="button" data-m393-section="m393DrillPages" data-label="Page Tools" aria-expanded="false">Page Tools  ›</button>',
      '  <div id="m393DrillPages" class="m393Section">',
      '    <div class="m393SectionTitle">Page Tools</div>',
      '    <div class="m393ActionGrid">',
      '      <button type="button" class="wide" data-m393-subpanel="m393DrillAdd" data-label="Add Page" aria-expanded="false">Add Page  ›</button>',
      '      <div id="m393DrillAdd" class="m393Subpanel">',
      '        <p class="m393SectionHelp">Add a blank page beside the current page.</p>',
      '        <div class="m393DirectionGrid">',
      '          <button type="button" class="m393Up" data-m393-add="up">↑ Add Above</button>',
      '          <button type="button" class="m393Left" data-m393-add="left">← Add Left</button>',
      '          <button type="button" class="m393Center m393Spacer" tabindex="-1" aria-hidden="true">Current</button>',
      '          <button type="button" class="m393Right" data-m393-add="right">Add Right →</button>',
      '          <button type="button" class="m393Down" data-m393-add="down">↓ Add Below</button>',
      '        </div>',
      '      </div>',
      '      <button type="button" data-m393-action="fitAll">Fit All Pages</button>',
      '      <button type="button" class="danger" data-m393-action="deletePage">Delete Current Page</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m393-section="m393DrillExport" data-label="Export & Share" aria-expanded="false">Export &amp; Share  ›</button>',
      '  <div id="m393DrillExport" class="m393Section">',
      '    <div class="m393SectionTitle">Export &amp; Share</div>',
      '    <div class="m393ActionGrid">',
      '      <button type="button" class="primary wide" data-m393-action="finish">Finish &amp; Send to Blaster</button>',
      '      <button type="button" data-m393-action="pdf">Download PDF</button>',
      '      <button type="button" data-m393-action="csv">Export CSV</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m393-section="m393DrillBackup" data-label="Backup & Restore" aria-expanded="false">Backup &amp; Restore  ›</button>',
      '  <div id="m393DrillBackup" class="m393Section">',
      '    <div class="m393SectionTitle">Backup &amp; Restore</div>',
      '    <p class="m393SectionHelp">Download a recovery copy or restore a previously saved Drill Log.</p>',
      '    <div class="m393ActionGrid">',
      '      <button type="button" data-m393-action="backup">Download Backup</button>',
      '      <button type="button" data-m393-action="restore">Restore Backup</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m393-section="m393DrillSettings" data-label="Settings" aria-expanded="false">Settings  ›</button>',
      '  <div id="m393DrillSettings" class="m393Section">',
      '    <div class="m393SectionTitle">Settings</div>',
      '    <div class="m393ActionGrid">',
      '      <button type="button" class="wide" data-m393-action="calibrate">Calibrate Employee / Job</button>',
      buildThemePickerHtml("m393DrillTheme"),
      '      <button id="mithrilUpdateMenuButton" type="button" class="wide" data-m393-action="updates">Check for Updates</button>',
      '    </div>',
      '    <div class="m393DangerZone"><button type="button" class="danger" data-m393-action="clear">Clear Drill Log Data</button></div>',
      '  </div>',
      '  <button id="mithrilHomeMenuButton" type="button" class="m393Home" data-m393-action="home">MITHRIL Home</button>',
      '</div>',
    ...
    wireAction(box, '[data-m393-action="close"]', closeMenu);
    wireAction(box, '[data-m393-action="info"]', function () { runAndClose("openInfo"); });
    wireAction(box, '[data-m393-action="editHoles"]', function () { closeMenu(); startDrillEditMode(); });
    wireAction(box, '[data-m393-action="fitAll"]', function () { runAndClose("fitAllPages"); });
    wireAction(box, '[data-m393-action="deletePage"]', function () { runAndClose("deletePage"); });
    wireAction(box, '[data-m393-action="finish"]', function () { runAndClose("finishAndSendToBlaster"); });
    wireAction(box, '[data-m393-action="pdf"]', function () { runAndClose("downloadPDF"); });
    wireAction(box, '[data-m393-action="csv"]', function () { runAndClose("exportCSV"); });
    wireAction(box, '[data-m393-action="backup"]', function () { runAndClose("downloadJSON"); });
    wireAction(box, '[data-m393-action="restore"]', function () {
      closeMenu();
      var input = byId("jsonInput");
      if (input) input.click();
    });
    wireAction(box, '[data-m393-action="calibrate"]', function () { runAndClose("startHeaderCalibration"); });
    wireAction(box, '[data-m393-action="updates"]', function () { runAndClose("checkUpdatesFromDrillLog"); });
    wireAction(box, '[data-m393-action="clear"]', function () { callGlobal("clearAll"); });
    wireAction(box, '[data-m393-action="home"]', function () { runAndClose("returnToSelector"); });
    wireThemeButtons(box);

    var addButtons = box.querySelectorAll("[data-m393-add]");
    for (var i = 0; i < addButtons.length; i += 1) {
      addButtons[i].addEventListener("click", function () {
        runAndClose("addPageAtDirection", [this.getAttribute("data-m393-add")]);
      });
    }
  }
  ...
  function patchShotMenu() {
    var menu = byId("menuModal");
    if (!menu || menu.getAttribute("data-m393-patched") === "shot") return;
    var box = menu.querySelector(".box");
    if (!box) return;

    menu.setAttribute("data-m393-patched", "shot");
    box.innerHTML = [
      '<div class="boxHead"><span>Shot Diagram Menu</span><button type="button" data-m393-action="close">Close</button></div>',
      '<p class="m393MenuIntro">Daily tools stay visible. Page layout, exports, backups, and setup tools open only when needed.</p>',
      '<div class="m393MenuStack">',
      '  <button type="button" data-m393-action="info">Shot Info</button>',
      '  <button type="button" class="primary" data-m393-action="editHoles">Edit Holes</button>',
      '  <button type="button" data-m393-section="m393ShotPages" data-label="Page Tools" aria-expanded="false">Page Tools  ›</button>',
      '  <div id="m393ShotPages" class="m393Section">',
      '    <div class="m393SectionTitle">Page Tools</div>',
      '    <div class="m393ActionGrid">',
      '      <button type="button" class="wide" data-m393-subpanel="m393ShotAdd" data-label="Add Page" aria-expanded="false">Add Page  ›</button>',
      '      <div id="m393ShotAdd" class="m393Subpanel">',
      '        <p class="m393SectionHelp">Add a blank page beside the current page.</p>',
      '        <div class="m393DirectionGrid">',
      '          <button type="button" class="m393Up" data-m393-add="up">↑ Add Above</button>',
      '          <button type="button" class="m393Left" data-m393-add="left">← Add Left</button>',
      ...
      '    </div>',
      '  </div>',
      '  <button type="button" data-m393-section="m393ShotExport" data-label="Export & Share" aria-expanded="false">Export &amp; Share  ›</button>',
      '  <div id="m393ShotExport" class="m393Section">',
      '    <div class="m393SectionTitle">Export &amp; Share</div>',
      '    <div class="m393ActionGrid">',
      '      <button type="button" class="primary wide" data-m393-action="finish">Finish &amp; Export PDF</button>',
      '      <button type="button" data-m393-action="shareCsv">Share CSV</button>',
      '      <button type="button" data-m393-action="csv">Download CSV</button>',
      '      <button type="button" class="wide" data-m393-action="pdf">Download PDF</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m393-section="m393ShotBackup" data-label="Backup & Restore" aria-expanded="false">Backup &amp; Restore  ›</button>',
      '  <div id="m393ShotBackup" class="m393Section">',
      '    <div class="m393SectionTitle">Backup &amp; Restore</div>',
      '    <p class="m393SectionHelp">Download a recovery copy or restore a previously saved Shot Diagram.</p>',
      '    <div class="m393ActionGrid">',
      '      <button type="button" data-m393-action="backup">Download Backup</button>',
      '      <button type="button" data-m393-action="restore">Restore Backup</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m393-section="m393ShotSettings" data-label="Settings" aria-expanded="false">Settings  ›</button>',
      '  <div id="m393ShotSettings" class="m393Section">',
      '    <div class="m393SectionTitle">Settings</div>',
      '    <div class="m393ActionGrid">',
      '      <button type="button" class="wide" data-m393-action="calibrate">Field Calibration</button>',
      buildThemePickerHtml("m393ShotTheme"),
      '      <button id="mithrilUpdateMenuButton" type="button" class="wide" data-m393-action="updates">Check for Updates</button>',
      '    </div>',
      '    <div class="m393DangerZone"><button type="button" class="danger" data-m393-action="clear">Clear Shot Data</button></div>',
      '  </div>',
      '  <button id="mithrilHomeMenuButton" type="button" class="m393Home" data-m393-action="home">MITHRIL Home</button>',
      '</div>',
    ...
    wireAction(box, '[data-m393-action="close"]', closeMenu);
    wireAction(box, '[data-m393-action="info"]', function () { runAndClose("openShotInfo"); });
    wireAction(box, '[data-m393-action="editHoles"]', function () { closeMenu(); startShotEditMode(); });
    wireAction(box, '[data-m393-action="fitAll"]', function () { runAndClose("fitAllPages"); });
    wireAction(box, '[data-m393-action="deletePage"]', function () { runAndClose("deleteCurrentPage"); });
    wireAction(box, '[data-m393-action="finish"]', function () { runAndClose("finishAndSend"); });
    wireAction(box, '[data-m393-action="shareCsv"]', function () { runAndClose("emailCSV"); });
    wireAction(box, '[data-m393-action="csv"]', function () { runAndClose("exportCSV"); });
    wireAction(box, '[data-m393-action="pdf"]', function () { runAndClose("exportPDFReport"); });
    wireAction(box, '[data-m393-action="backup"]', function () { runAndClose("downloadJSON"); });
    wireAction(box, '[data-m393-action="restore"]', function () { runAndClose("triggerLoadJSON"); });
    wireAction(box, '[data-m393-action="calibrate"]', function () { runAndClose("openFieldCalibration"); });
    wireAction(box, '[data-m393-action="updates"]', checkShotUpdates);
    wireAction(box, '[data-m393-action="clear"]', function () { callGlobal("clearAll"); });
    wireAction(box, '[data-m393-action="home"]', function () { closeMenu(); homeFromShot(); });
    wireThemeButtons(box);
  ...
  function injectMultiQuickStyles() {
    if (byId("mithrilMultiQuickM393Styles")) return;
    var style = document.createElement("style");
    style.id = "mithrilMultiQuickM393Styles";
    style.textContent = [
      ".m393QuickIntro{margin:0 0 12px;color:#444;font-size:13px;font-weight:750;line-height:1.4}",
      ".m393QuickRows{display:grid;gap:10px}",
      ".m393QuickRow{display:grid;grid-template-columns:78px minmax(130px,1.15fr) minmax(110px,.85fr);gap:8px;align-items:end;padding:9px;border:1px solid #bbb;border-radius:10px;background:#f8f8f8}",
      ".m393QuickRow.inactive{opacity:.68}",
      ".m393QuickUse{display:flex;align-items:center;justify-content:center;gap:6px;min-height:46px;padding:6px;border:1px solid #aaa;border-radius:8px;background:#fff;font-size:13px;font-weight:950}",
      ".m393QuickUse input{width:24px;height:24px;min-height:24px;margin:0;padding:0}",
      ".m393QuickRow label{min-width:0}",
      ".m393QuickRow select,.m393QuickRow input{width:100%;min-height:46px;font-size:17px;padding:8px;border:1px solid #999;border-radius:8px;background:#fff}",
      ".m393QuickValueCell{min-width:0}",
      ".m393QuickActions{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:12px}",
      ".m393QuickActions button{min-height:48px}",
      ".m393QuickBarSummary{min-width:0;font-size:14px;font-weight:950;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
      ".m393QuickBarHint{grid-column:1/-1;font-size:12px;font-weight:850;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#333}",
      "#quickBar.m393MultiQuickBar,#singleFillBar.m393MultiQuickBar{grid-template-columns:minmax(0,1fr) auto auto!important}",
      "#quickModal .box.m393QuickModalBox{width:min(650px,96vw)}",
      "#quickModal .box.m393QuickKeypadOpen{padding-bottom:365px!important}",
      "@media(max-width:520px){.m393QuickRow{grid-template-columns:68px 1fr}.m393QuickValueCell{grid-column:2}.m393QuickActions{grid-template-columns:1fr}.m393QuickBarSummary{font-size:13px}}"
    ].join("");
    document.head.appendChild(style);
  }
  ...
  function buildMultiQuickRows(prefix, fields) {
    ...
    for (var i = 1; i <= 3; i += 1) {
      html += [
        '<div id="' + prefix + 'Row' + i + '" class="m393QuickRow">',
        '  <label class="m393QuickUse"><input id="' + prefix + 'Use' + i + '" type="checkbox" /> Use ' + i + '</label>',
        '  <label>Field<select id="' + prefix + 'Field' + i + '">' + options + '</select></label>',
        '  <label class="m393QuickValueCell">Value',
  ...
  function installCanvasBackgroundBridge(canvas) {
    if (!canvas || canvas.getAttribute("data-m393-theme-canvas") === "true") return;
    var context = canvas.getContext && canvas.getContext("2d");
    if (!context || context.__mithrilM393FillRect) return;
    var originalFillRect = context.fillRect.bind(context);
    context.__mithrilM393FillRect = originalFillRect;
    ...
    canvas.setAttribute("data-m393-theme-canvas", "true");
  }
  ...
  function drillQuickPrefix() { return "m393DrillQuick"; }
  ...
  function updateDrillMultiQuickBar(message) {
    ...
    var summary = byId("m393DrillQuickSummary");
    var hint = byId("m393DrillQuickHint");
  ...
  function installDrillQuickPadSupport() {
    if (window.__mithrilM393DrillPad) return;
    window.__mithrilM393DrillPad = true;
    ...
    window.showPad = function (id) {
      ...
      if (box && byId("quickModal").classList.contains("show")) box.classList.add("m393QuickKeypadOpen");
    ...
    window.hidePad = function () {
      ...
      var box = byId("quickModal") && byId("quickModal").querySelector(".box");
      if (box) box.classList.remove("m393QuickKeypadOpen");
    };
  ...
  function patchDrillMultiQuick() {
    ...
    var bar = byId("quickBar");
    if (!modal || !bar || modal.getAttribute("data-m393-multi-quick") === "drill") return;
    modal.setAttribute("data-m393-multi-quick", "drill");
    var box = modal.querySelector(".box");
    if (!box) return;
    box.classList.add("m393QuickModalBox");
    box.innerHTML = [
      '<div class="boxHead"><span>Quick Fill</span><button type="button" id="m393DrillQuickClose">Close</button></div>',
      '<p class="m393QuickIntro">Use up to three different fields. One tap on a hole applies every active row together.</p>',
      '<div class="m393QuickRows">' + buildMultiQuickRows(drillQuickPrefix(), DRILL_MULTI_QUICK_FIELDS) + '</div>',
      '<div class="m393QuickActions">',
      '  <button type="button" class="primary" id="m393DrillQuickOn">Turn On</button>',
      '  <button type="button" id="m393DrillQuickOff">Turn Off</button>',
      '  <button type="button" class="danger" id="m393DrillQuickClear">Clear Values</button>',
      '</div>'
    ].join("");

    bar.classList.add("m393MultiQuickBar");
    bar.innerHTML = [
      '<div id="m393DrillQuickSummary" class="m393QuickBarSummary"></div>',
      '<button type="button" id="m393DrillQuickEdit">Edit</button>',
      '<button type="button" class="danger" id="m393DrillQuickBarOff">Off</button>',
      '<div id="m393DrillQuickHint" class="m393QuickBarHint"></div>'
    ].join("");
    ...
    byId("m393DrillQuickClose").addEventListener("click", function () { window.closeQuickModal(); });
    byId("m393DrillQuickOn").addEventListener("click", function () { saveDrillMultiQuick(true); });
    byId("m393DrillQuickOff").addEventListener("click", function () { saveDrillMultiQuick(false); });
    byId("m393DrillQuickClear").addEventListener("click", function () {
      for (var i = 1; i <= 3; i += 1) {
    ...
    byId("m393DrillQuickEdit").addEventListener("click", function () { window.openQuickModal(); });
    byId("m393DrillQuickBarOff").addEventListener("click", function () { window.turnQuickOff(); });
  ...
  function shotQuickPrefix() { return "m393ShotQuick"; }
  ...
  function updateShotMultiQuickBar(message) {
    ...
    if (!bar) return;
    bar.classList.toggle("show", !!state.enabled);
    var summary = byId("m393ShotQuickSummary");
    var hint = byId("m393ShotQuickHint");
  ...
  function installShotQuickKeypadSupport() {
    if (window.__mithrilM393ShotPad) return;
    window.__mithrilM393ShotPad = true;
    ...
    window.scrollActiveLoadInputIntoView = function (id) {
      ...
      if (customIndex(id)) {
        var box = byId("quickModal") && byId("quickModal").querySelector(".box");
        if (box) box.classList.add("m393QuickKeypadOpen");
      }
    };
  ...
  function patchShotMultiQuick() {
    var modal = byId("quickModal");
    var bar = byId("singleFillBar");
    if (!modal || !bar || modal.getAttribute("data-m393-multi-quick") === "shot") return;
    modal.setAttribute("data-m393-multi-quick", "shot");
    var box = modal.querySelector(".box");
    if (!box) return;
    box.classList.add("m393QuickModalBox");
    box.innerHTML = [
      '<div class="boxHead"><span>Quick Fill</span><button type="button" id="m393ShotQuickClose">Close</button></div>',
      '<p class="m393QuickIntro">Use up to three different fields. One tap on a hole applies every active row together.</p>',
      '<div class="m393QuickRows">' + buildMultiQuickRows(shotQuickPrefix(), SHOT_MULTI_QUICK_FIELDS) + '</div>',
      '<div class="m393QuickActions">',
      '  <button type="button" class="primary" id="m393ShotQuickOn">Turn On</button>',
      '  <button type="button" id="m393ShotQuickOff">Turn Off</button>',
      '  <button type="button" class="danger" id="m393ShotQuickClear">Clear Values</button>',
      '</div>'
    ].join("");

    bar.classList.add("m393MultiQuickBar");
    bar.innerHTML = [
      '<div id="m393ShotQuickSummary" class="m393QuickBarSummary"></div>',
      '<button type="button" id="m393ShotQuickEdit">Edit</button>',
      '<button type="button" class="danger" id="m393ShotQuickBarOff">Off</button>',
      '<div id="m393ShotQuickHint" class="m393QuickBarHint"></div>'
    ].join("");
    ...
    byId("m393ShotQuickClose").addEventListener("click", function () { window.closeQuickEntry(); });
    byId("m393ShotQuickOn").addEventListener("click", function () { saveShotMultiQuick(true); });
    byId("m393ShotQuickOff").addEventListener("click", function () { saveShotMultiQuick(false); });
    byId("m393ShotQuickClear").addEventListener("click", function () {
      for (var i = 1; i <= 3; i += 1) {
        byId(prefix + "Value" + i).value = "";
        byId(prefix + "Bool" + i).value = "Yes";
      }
    ...
    byId("m393ShotQuickEdit").addEventListener("click", function () { window.openQuickEntry(); });
    byId("m393ShotQuickBarOff").addEventListener("click", function () { window.quickEnabledOff(); });
  ...
  function injectGPSStyles() {
    if (byId("mithrilGPSM393Styles")) return;
    var style = document.createElement("style");
    style.id = "mithrilGPSM393Styles";
    style.textContent = [
      ".m393GPSDetails{margin-top:12px;border:1px solid #9ab5d6;border-radius:10px;background:#f6faff;overflow:hidden}",
      ".m393GPSDetails summary{min-height:48px;display:flex;align-items:center;padding:8px 11px;font-size:15px;font-weight:950;cursor:pointer;user-select:none}",
      ".m393GPSBody{padding:0 10px 10px}",
      ".m393GPSGrid{display:grid;grid-template-columns:1fr 1fr;gap:9px}",
      ".m393GPSGrid label{min-width:0}",
      ".m393GPSGrid input{width:100%;min-height:44px;font-size:17px;padding:8px;border:1px solid #999;border-radius:8px;box-sizing:border-box}",
      ".m393GPSGrid .wide{grid-column:1/-1}",
      ".m393GPSActions{display:grid;grid-template-columns:1fr;gap:8px;margin-top:9px}",
      ".m393GPSActions button{min-height:46px}",
      ".m393GPSStatus{margin-top:8px;min-height:18px;font-size:12px;font-weight:800;color:#365b82;line-height:1.3}",
      ".m393GPSMenuGrid{display:grid;grid-template-columns:1fr 1fr;gap:8px}",
      ".m393GPSMenuGrid button{min-height:48px}",
      ".m393GPSMenuGrid .wide{grid-column:1/-1}",
      ".m393GPSArrangeBar{display:none;position:fixed;left:8px;right:8px;bottom:8px;z-index:240;grid-template-columns:minmax(0,1fr) auto auto;gap:8px;align-items:center;background:rgba(255,255,255,.98);border:2px solid #8a4fff;border-radius:12px;padding:8px;box-shadow:0 5px 18px rgba(0,0,0,.38)}",
      ".m393GPSArrangeBar.show{display:grid}",
      ".m393GPSArrangeHint{font-size:13px;line-height:1.25;font-weight:900;color:#333;min-width:0}",
      ".m393GPSArrangeBar button{min-height:44px}",
      "@media(max-width:520px){.m393GPSGrid{grid-template-columns:1fr}.m393GPSGrid .wide{grid-column:auto}.m393GPSActions{grid-template-columns:1fr}.m393GPSMenuGrid{grid-template-columns:1fr}.m393GPSMenuGrid .wide{grid-column:auto}.m393GPSArrangeBar{grid-template-columns:1fr 1fr}.m393GPSArrangeHint{grid-column:1/-1}}"
    ].join("");
    document.head.appendChild(style);
  }
  ...
  function patchGPSDrawing(type) {
    if (window.__mithrilM393GPSDrawing) return;
    window.__mithrilM393GPSDrawing = true;
  ...
  function gpsEditorPrefix(type) {
    return type === "drill" ? "m393DrillGPS" : "m393ShotGPS";
  }
  ...
  function installGPSHoleEditor(type) {
    ...
    if (!modal || modal.getAttribute("data-m393-gps-editor") === type) return;
    ...
    details.className = "m393GPSDetails";
    details.innerHTML = [
      '<summary>Manual GPS Tag (optional)</summary>',
      '<div class="m393GPSBody">',
      '  <p class="m393GPSStatus">Enter coordinates collected with your external GPS equipment. MITHRIL does not request or capture this device\'s location.</p>',
      '  <div class="m393GPSGrid">',
      ...
      '  </div>',
      '  <div class="m393GPSActions">',
      '    <button type="button" class="danger" id="' + prefix + 'Clear">Clear GPS Tag</button>',
      '  </div>',
      '  <div id="' + prefix + 'Status" class="m393GPSStatus">No manual GPS tag saved for this hole.</div>',
      '</div>'
    ].join("");
    ...
    modal.setAttribute("data-m393-gps-editor", type);
  }
  ...
  function updateGPSMenuState(type) {
    var button = byId(type === "drill" ? "m393DrillGPSToggle" : "m393ShotGPSToggle");
    var count = byId(type === "drill" ? "m393DrillGPSCount" : "m393ShotGPSCount");
  ...
  function ensureGPSArrangeBar(type) {
    var bar = byId("m393GPSArrangeBar");
    if (!bar) {
      bar = document.createElement("div");
      bar.id = "m393GPSArrangeBar";
      bar.className = "m393GPSArrangeBar";
      bar.innerHTML = [
        '<div id="m393GPSArrangeHint" class="m393GPSArrangeHint">Drag GPS boxes to move them. The arrows follow automatically.</div>',
        '<button type="button" id="m393GPSArrangeReset">Reset Page</button>',
        '<button type="button" class="primary" id="m393GPSArrangeDone">Done</button>'
      ].join("");
      document.body.appendChild(bar);
      byId("m393GPSArrangeDone").addEventListener("click", function () { finishGPSArrange(); });
      byId("m393GPSArrangeReset").addEventListener("click", function () { resetGPSPositions(gpsArrangeType || type); });
    }
    return bar;
  }
  ...
  function startGPSArrange(type) {
    ...
    byId("m393GPSArrangeHint").textContent = "Drag GPS boxes on any visible page. Touching a box automatically activates its page.";
    if (typeof window.draw === "function") window.draw();
  }
  ...
  function buildGPSMenuHtml(type) {
    var prefix = type === "drill" ? "m393DrillGPS" : "m393ShotGPS";
    return [
      '<button type="button" class="wide" data-m393-subpanel="' + prefix + 'Panel" data-label="GPS Callouts" aria-expanded="false">GPS Callouts  ›</button>',
      '<div id="' + prefix + 'Panel" class="m393Subpanel">',
      '  <p class="m393SectionHelp">Coordinates entered manually are stored on each hole. Visible callouts and arrows are also included in PDF exports.</p>',
      '  <div id="' + prefix + 'Count" class="m393GPSStatus"></div>',
      '  <div class="m393GPSMenuGrid">',
  ...
  function installGPSMenuTools(type) {
    var settings = byId(type === "drill" ? "m393DrillSettings" : "m393ShotSettings");
    if (!settings || settings.getAttribute("data-m393-gps-menu") === "true") return;
    var grid = settings.querySelector(".m393ActionGrid");
    if (!grid) return;
    ...
    settings.setAttribute("data-m393-gps-menu", "true");
    updateGPSMenuState(type);
    if (!window.__mithrilM393GPSMenuOpenWrapped && typeof window.openMenu === "function") {
      var originalOpenMenuForGPS = window.openMenu;
      window.openMenu = function () {
        updateGPSMenuState(type);
        return originalOpenMenuForGPS.apply(this, arguments);
      };
      window.__mithrilM393GPSMenuOpenWrapped = true;
    }
  }
  ...
  function activateGPSPage(type, pageNum) {
    ...
    try {
      ...
      var hint = byId("m393GPSArrangeHint");
  ...
  function installGPSDragging(canvas, type) {
    if (!canvas || canvas.getAttribute("data-m393-gps-drag") === "true") return;
    ...
    canvas.setAttribute("data-m393-gps-drag", "true");
  }
  ...
  function injectHoleConditionStyles() {
    if (byId("mithrilHoleConditionsM393Styles")) return;
    var style = document.createElement("style");
    style.id = "mithrilHoleConditionsM393Styles";
    style.textContent = [
      ".m393ConditionEditButton{grid-column:1/-1;min-height:46px;background:#fff8c9;border-color:#c8a600;color:#3d3300}",
      ".m393ConditionEditButton.hidden{display:none}",
      "#m393ConditionModal{z-index:260}",
      "#m393ConditionModal .box{width:min(720px,97vw)}",
      ".m393ConditionIntro{margin:0 0 10px;color:#444;font-size:13px;font-weight:750;line-height:1.4}",
      ".m393ConditionRows{display:grid;gap:10px}",
      ".m393ConditionRow{position:relative;display:grid;grid-template-columns:minmax(145px,1.25fr) minmax(100px,.7fr) minmax(120px,.85fr);gap:8px;padding:10px;border:1px solid #c8ad44;border-radius:11px;background:#fffbea}",
      ".m393ConditionRow label{min-width:0}",
      ".m393ConditionRow select,.m393ConditionRow input[type=number],.m393ConditionRow input[type=text]{width:100%;min-height:44px;padding:8px;border:1px solid #999;border-radius:8px;background:#fff;font-size:17px}",
      ".m393ConditionBottom{grid-column:1/3;display:flex;align-items:center;gap:9px;min-height:44px;padding:7px 9px;border:1px solid #aaa;border-radius:8px;background:#fff;font-size:14px;font-weight:900}",
      ".m393ConditionBottom input{width:25px;height:25px;min-height:25px;margin:0}",
      ".m393ConditionRemove{align-self:end;min-height:44px;background:#fff1f1;border-color:#c66}",
      ".m393ConditionOther{grid-column:1/-1}",
      ".m393ConditionOther.hidden{display:none}",
      ".m393ConditionTools{display:grid;grid-template-columns:1fr auto;gap:8px;margin-top:10px}",
      ".m393ConditionPreview{margin-top:10px;padding:10px;border:1px solid #9f8a2d;border-radius:9px;background:#fffdf3;font-size:14px;font-weight:850;line-height:1.4;color:#342d0b}",
      ".m393ConditionActions{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:12px}",
      ".m393ConditionActions button{min-height:48px}",
      "@media(max-width:620px){.m393ConditionRow{grid-template-columns:1fr 1fr}.m393ConditionRow>label:first-child{grid-column:1/-1}.m393ConditionBottom{grid-column:1/-1}.m393ConditionRemove{grid-column:1/-1}.m393ConditionActions{grid-template-columns:1fr}.m393ConditionTools{grid-template-columns:1fr}}"
    ].join("");
    document.head.appendChild(style);
  }

  function ensureHoleConditionModal() {
    var modal = byId("m393ConditionModal");
    if (modal) return modal;
    modal = document.createElement("div");
    modal.id = "m393ConditionModal";
    modal.className = "modal";
    modal.innerHTML = [
      '<div class="box">',
      '  <div class="boxHead"><span id="m393ConditionTitle">Hole Conditions</span><button type="button" id="m393ConditionClose">Close</button></div>',
      '  <p class="m393ConditionIntro">Record where a loading condition begins and where competent rock resumes. These intervals are informational and never reduce Total Rock.</p>',
      '  <div id="m393ConditionRows" class="m393ConditionRows"></div>',
      '  <div class="m393ConditionTools"><button type="button" id="m393ConditionAdd">+ Add another condition</button><button type="button" id="m393ConditionClear" class="danger">Clear all</button></div>',
      '  <div id="m393ConditionPreview" class="m393ConditionPreview">No condition intervals entered.</div>',
      '  <div class="m393ConditionActions"><button type="button" class="primary" id="m393ConditionSave">Save Conditions</button><button type="button" id="m393ConditionCancel">Cancel</button><button type="button" id="m393ConditionDoneHole">Save Conditions + Hole</button></div>',
      '</div>'
    ].join("");
    document.body.appendChild(modal);

    byId("m393ConditionClose").addEventListener("click", cancelHoleConditionModal);
    byId("m393ConditionCancel").addEventListener("click", cancelHoleConditionModal);
    byId("m393ConditionAdd").addEventListener("click", function () {
    ...
    byId("m393ConditionClear").addEventListener("click", function () {
      conditionDraft = [];
      renderHoleConditionRows();
    });
    byId("m393ConditionSave").addEventListener("click", function () { saveHoleConditionModal(false); });
    byId("m393ConditionDoneHole").addEventListener("click", function () { if (saveHoleConditionModal(false)) window.saveHole(false); });
    byId("m393ConditionRows").addEventListener("input", readConditionRowsIntoDraft);
    byId("m393ConditionRows").addEventListener("change", function (event) {
      readConditionRowsIntoDraft();
      if (event.target && event.target.classList.contains("m393ConditionType")) renderHoleConditionRows();
      else updateHoleConditionPreview();
    });
    byId("m393ConditionRows").addEventListener("click", function (event) {
  ...
  function readConditionRowsIntoDraft() {
    var rows = document.querySelectorAll("#m393ConditionRows .m393ConditionRow");
    var next = [];
    for (var i = 0; i < rows.length; i += 1) {
      var row = rows[i];
      var type = row.querySelector(".m393ConditionType");
      var other = row.querySelector(".m393ConditionOtherInput");
      var start = row.querySelector(".m393ConditionStart");
      var end = row.querySelector(".m393ConditionEnd");
      var bottom = row.querySelector(".m393ConditionToBottom");
      next.push({
  ...
  function renderHoleConditionRows() {
    var container = byId("m393ConditionRows");
    if (!container) return;
    var html = "";
    for (var i = 0; i < conditionDraft.length; i += 1) {
      var entry = conditionDraft[i];
      var otherClass = entry.type === "Other" ? "m393ConditionOther" : "m393ConditionOther hidden";
      html += [
        '<div class="m393ConditionRow" data-condition-index="' + i + '">',
        '  <label>Condition<select class="m393ConditionType">' + conditionOptionsHtml(entry.type) + '</select></label>',
        '  <label>Starts at (ft)<input class="m393ConditionStart" type="number" inputmode="decimal" step="0.1" min="0" value="' + String(entry.start || "") + '" /></label>',
        '  <label>Competent rock resumes (ft)<input class="m393ConditionEnd" type="number" inputmode="decimal" step="0.1" min="0" value="' + String(entry.end || "") + '"' + (entry.toBottom ? ' disabled' : '') + ' /></label>',
        '  <label class="m393ConditionBottom"><input class="m393ConditionToBottom" type="checkbox"' + (entry.toBottom ? ' checked' : '') + ' /> Continues to bottom</label>',
        '  <button type="button" class="m393ConditionRemove" data-condition-remove="' + i + '">Remove</button>',
        '  <label class="' + otherClass + '">Other condition name<input class="m393ConditionOtherInput" type="text" value="' + String(entry.other || "").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;") + '" /></label>',
        '</div>'
      ].join("");
    }
    if (!conditionDraft.length) html = '<div class="m393ConditionPreview">No condition intervals. Tap “Add another condition” to begin.</div>';
  ...
  function updateConditionEditorControls() {
    var rows = document.querySelectorAll("#m393ConditionRows .m393ConditionRow");
    for (var i = 0; i < rows.length; i += 1) {
      var bottom = rows[i].querySelector(".m393ConditionToBottom");
      var end = rows[i].querySelector(".m393ConditionEnd");
      var type = rows[i].querySelector(".m393ConditionType");
      var otherWrap = rows[i].querySelector(".m393ConditionOther");
      if (end && bottom) {
  ...
  function updateHoleConditionPreview() {
    var preview = byId("m393ConditionPreview");
    if (!preview) return;
  ...
  function openHoleConditionModal(createDefault) {
    ...
    byId("m393ConditionTitle").textContent = "Hole Conditions — " + (typeof currentHole !== "undefined" ? currentHole : "Hole");
    renderHoleConditionRows();
    byId("m393ConditionModal").classList.add("show");
    window.setTimeout(function () {
      var input = document.querySelector("#m393ConditionRows .m393ConditionStart");
  ...
  function updateHoleConditionButton() {
    var button = byId("m393EditHoleConditions");
    var checkbox = byId("breakthrough");
  ...
  function installDrillHoleConditions() {
    if (window.__mithrilM393HoleConditions) return;
    var checkbox = byId("breakthrough");
    var holeBox = byId("holeBox");
    if (!checkbox || !holeBox) return;
    window.__mithrilM393HoleConditions = true;
    ...
    edit.id = "m393EditHoleConditions";
    edit.type = "button";
    edit.className = "m393ConditionEditButton hidden";
  ...
  function patchDrillConditionCSV() {
    if (window.__mithrilM393ConditionCSV || typeof window.exportCSV !== "function") return;
    window.__mithrilM393ConditionCSV = true;
    window.exportCSV = function () {
  ...
  // ---------------------------------------------------------------------------
  // m39.3 Drill Log selection, clipboard, and cross-page transform engine.
  ...
  function drillEditSetHint(message) {
    var hint = byId("m393DrillEditHint");
  ...
  function drillUpdateEditBar() {
    var count = drillEditSelectionList().length;
    var status = byId("m393DrillEditStatus");
    if (status) status.textContent = drillSelectionDescription();
    var modes = ["hole", "row", "column"];
    for (var i = 0; i < modes.length; i += 1) {
      var button = byId("m393DrillMode" + modes[i].charAt(0).toUpperCase() + modes[i].slice(1));
      if (button) button.classList.toggle("active", drillEditSelectMode === modes[i]);
    }
    var hasSelection = count > 0;
    var copyButton = byId("m393DrillCopy");
    var cutButton = byId("m393DrillCut");
    var pasteButton = byId("m393DrillPaste");
    ...
    var bar = byId("m393DrillEditBar");
    var arrows = bar ? bar.querySelectorAll("[data-m393-drill-shift]") : [];
    for (var a = 0; a < arrows.length; a += 1) arrows[a].disabled = !hasSelection;
    var undoButton = byId("m393DrillUndo");
    if (undoButton) undoButton.disabled = !drillEditUndoHistory.length;
  }

  function injectDrillEditStyles() {
    if (byId("mithrilDrillEditM393Styles")) return;
    var style = document.createElement("style");
    style.id = "mithrilDrillEditM393Styles";
    style.textContent = [
      ".m393DrillEditBar{display:none;position:fixed;left:8px;right:8px;bottom:8px;z-index:245;background:rgba(255,255,255,.985);border:2px solid #8a4fff;border-radius:13px;padding:8px;box-shadow:0 6px 22px rgba(0,0,0,.42);gap:7px}",
      ".m393DrillEditBar.show{display:grid}",
      ".m393DrillEditHead{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:center}",
      ".m393DrillEditTitle{font-size:14px;font-weight:950;color:#222}",
      ".m393DrillEditStatus{font-size:12px;font-weight:850;color:#5d3b96;margin-top:2px}",
      ".m393DrillEditModes,.m393DrillEditActions{display:grid;grid-template-columns:repeat(4,1fr);gap:6px}",
      ".m393DrillEditModes button,.m393DrillEditActions button{min-height:43px;padding:5px;font-size:13px}",
      ".m393DrillEditModes button.active{background:#8a4fff;color:#fff;border-color:#6f35da}",
      ".m393DrillEditDirections{display:grid;grid-template-columns:repeat(5,1fr);gap:6px}",
      ".m393DrillEditDirections button{min-height:45px;font-size:18px;padding:4px}",
      ".m393DrillEditHint{min-height:17px;font-size:11px;line-height:1.25;font-weight:800;color:#444}",
      ".m393DrillEditDone{min-width:76px;background:#1f6feb;color:#fff;border-color:#1f6feb}",
      "@media(max-width:520px){.m393DrillEditModes,.m393DrillEditActions{grid-template-columns:repeat(2,1fr)}.m393DrillEditDirections{grid-template-columns:repeat(5,1fr)}.m393DrillEditDirections button{font-size:17px}.m393DrillEditHead{grid-template-columns:1fr auto}}"
    ].join("");
    document.head.appendChild(style);
  }
  ...
  function ensureDrillEditBar() {
    ...
    if (bar) return bar;
    bar = document.createElement("div");
    bar.id = "m393DrillEditBar";
    bar.className = "m393DrillEditBar";
    bar.innerHTML = [
      '<div class="m393DrillEditHead">',
      '  <div><div class="m393DrillEditTitle">Edit Drill Holes</div><div id="m393DrillEditStatus" class="m393DrillEditStatus">0 selected</div></div>',
      '  <button type="button" id="m393DrillEditDone" class="m393DrillEditDone">Done</button>',
      '</div>',
      '<div class="m393DrillEditModes">',
      '  <button type="button" id="m393DrillModeHole">Hole</button>',
      '  <button type="button" id="m393DrillModeRow">Row</button>',
      '  <button type="button" id="m393DrillModeColumn">Column</button>',
      '  <button type="button" id="m393DrillSelectAll">All Page</button>',
      '</div>',
      '<div class="m393DrillEditActions">',
      '  <button type="button" id="m393DrillCopy">Copy</button>',
      '  <button type="button" id="m393DrillCut">Cut</button>',
      '  <button type="button" id="m393DrillPaste">Paste</button>',
      '  <button type="button" id="m393DrillClearSelection">Clear Selection</button>',
      '</div>',
      '<div class="m393DrillEditDirections">',
      '  <button type="button" data-m393-drill-shift="left" aria-label="Shift left">←</button>',
      '  <button type="button" data-m393-drill-shift="up" aria-label="Shift up">↑</button>',
      '  <button type="button" id="m393DrillUndo" aria-label="Undo">↶</button>',
      '  <button type="button" data-m393-drill-shift="down" aria-label="Shift down">↓</button>',
      '  <button type="button" data-m393-drill-shift="right" aria-label="Shift right">→</button>',
      '</div>',
      '<div id="m393DrillEditHint" class="m393DrillEditHint">Tap saved holes to select them. Pan and pinch zoom still work.</div>'
    ].join("");
    document.body.appendChild(bar);

    byId("m393DrillEditDone").addEventListener("click", finishDrillEditMode);
    byId("m393DrillModeHole").addEventListener("click", function () { drillSetSelectionMode("hole"); });
    byId("m393DrillModeRow").addEventListener("click", function () { drillSetSelectionMode("row"); });
    byId("m393DrillModeColumn").addEventListener("click", function () { drillSetSelectionMode("column"); });
    byId("m393DrillSelectAll").addEventListener("click", drillSelectAllCurrentPage);
    byId("m393DrillCopy").addEventListener("click", function () { drillBuildClipboard("copy"); });
    byId("m393DrillCut").addEventListener("click", function () { drillBuildClipboard("cut"); });
    byId("m393DrillPaste").addEventListener("click", drillArmPaste);
    byId("m393DrillClearSelection").addEventListener("click", function () {
      drillEditSelection = {};
      drillEditPasteArmed = false;
      draw();
      drillEditSetHint("Selection cleared.");
    });
    byId("m393DrillUndo").addEventListener("click", drillUndoLastEdit);
    var arrows = bar.querySelectorAll("[data-m393-drill-shift]");
    for (var i = 0; i < arrows.length; i += 1) {
      arrows[i].addEventListener("click", function () { drillShiftSelection(this.getAttribute("data-m393-drill-shift")); });
    }
    return bar;
  }
  ...
  function patchDrillEditDrawing() {
    if (window.__mithrilM393DrillEditDrawing) return;
    window.__mithrilM393DrillEditDrawing = true;
  ...
  function installDrillEditInteraction(canvas) {
    if (!canvas || canvas.getAttribute("data-m393-drill-edit") === "true") return;
    canvas.setAttribute("data-m393-drill-edit", "true");
  ...
  function shotEditSetHint(message) {
    var hint = byId("m393ShotEditHint");
    if (hint) hint.textContent = message || "";
    shotUpdateEditBar();
  }
  ...
  function shotUpdateEditBar() {
    var count = shotEditSelectionList().length;
    var status = byId("m393ShotEditStatus");
    if (status) status.textContent = shotSelectionDescription();
    var modes = ["hole", "row", "column"];
    for (var i = 0; i < modes.length; i += 1) {
      var button = byId("m393ShotMode" + modes[i].charAt(0).toUpperCase() + modes[i].slice(1));
      if (button) button.classList.toggle("active", shotEditSelectMode === modes[i]);
    }
    var hasSelection = count > 0;
    var copyButton = byId("m393ShotCopy");
    var cutButton = byId("m393ShotCut");
    var pasteButton = byId("m393ShotPaste");
    if (copyButton) copyButton.disabled = !hasSelection;
    if (cutButton) cutButton.disabled = !hasSelection;
    if (pasteButton) pasteButton.disabled = !(shotEditClipboard && shotEditClipboard.items && shotEditClipboard.items.length);
    var arrows = document.querySelectorAll("[data-m393-edit-shift]");
    for (var a = 0; a < arrows.length; a += 1) arrows[a].disabled = !hasSelection;
    var undoButton = byId("m393ShotUndo");
    if (undoButton) undoButton.disabled = !shotEditUndoHistory.length;
  }

  function injectShotEditStyles() {
    if (byId("mithrilShotEditM393Styles")) return;
    var style = document.createElement("style");
    style.id = "mithrilShotEditM393Styles";
    style.textContent = [
      ".m393ShotEditBar{display:none;position:fixed;left:8px;right:8px;bottom:8px;z-index:245;background:rgba(255,255,255,.985);border:2px solid #8a4fff;border-radius:13px;padding:8px;box-shadow:0 6px 22px rgba(0,0,0,.42);gap:7px}",
      ".m393ShotEditBar.show{display:grid}",
      ".m393ShotEditHead{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:center}",
      ".m393ShotEditTitle{font-size:14px;font-weight:950;color:#222}",
      ".m393ShotEditStatus{font-size:12px;font-weight:850;color:#5d3b96;margin-top:2px}",
      ".m393ShotEditModes,.m393ShotEditActions{display:grid;grid-template-columns:repeat(4,1fr);gap:6px}",
      ".m393ShotEditModes button,.m393ShotEditActions button{min-height:43px;padding:5px;font-size:13px}",
      ".m393ShotEditModes button.active{background:#8a4fff;color:#fff;border-color:#6f35da}",
      ".m393ShotEditDirections{display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr;gap:6px}",
      ".m393ShotEditDirections button{min-height:45px;font-size:18px;padding:4px}",
      ".m393ShotEditHint{min-height:17px;font-size:11px;line-height:1.25;font-weight:800;color:#444}",
      ".m393ShotEditDone{min-width:76px;background:#1f6feb;color:#fff;border-color:#1f6feb}",
      "@media(max-width:520px){.m393ShotEditModes,.m393ShotEditActions{grid-template-columns:repeat(2,1fr)}.m393ShotEditDirections{grid-template-columns:repeat(5,1fr)}.m393ShotEditDirections button{font-size:17px}.m393ShotEditHead{grid-template-columns:1fr auto}}"
    ].join("");
    document.head.appendChild(style);
  }

  function ensureShotEditBar() {
    var bar = byId("m393ShotEditBar");
    if (bar) return bar;
    bar = document.createElement("div");
    bar.id = "m393ShotEditBar";
    bar.className = "m393ShotEditBar";
    bar.innerHTML = [
      '<div class="m393ShotEditHead">',
      '  <div><div class="m393ShotEditTitle">Edit Holes</div><div id="m393ShotEditStatus" class="m393ShotEditStatus">0 selected</div></div>',
      '  <button type="button" id="m393ShotEditDone" class="m393ShotEditDone">Done</button>',
      '</div>',
      '<div class="m393ShotEditModes">',
      '  <button type="button" id="m393ShotModeHole">Hole</button>',
      '  <button type="button" id="m393ShotModeRow">Row</button>',
      '  <button type="button" id="m393ShotModeColumn">Column</button>',
      '  <button type="button" id="m393ShotSelectAll">All Page</button>',
      '</div>',
      '<div class="m393ShotEditActions">',
      '  <button type="button" id="m393ShotCopy">Copy</button>',
      '  <button type="button" id="m393ShotCut">Cut</button>',
      '  <button type="button" id="m393ShotPaste">Paste</button>',
      '  <button type="button" id="m393ShotClearSelection">Clear Selection</button>',
      '</div>',
      '<div class="m393ShotEditDirections">',
      '  <button type="button" data-m393-edit-shift="left" aria-label="Shift left">←</button>',
      '  <button type="button" data-m393-edit-shift="up" aria-label="Shift up">↑</button>',
      '  <button type="button" id="m393ShotUndo" aria-label="Undo">↶</button>',
      '  <button type="button" data-m393-edit-shift="down" aria-label="Shift down">↓</button>',
      '  <button type="button" data-m393-edit-shift="right" aria-label="Shift right">→</button>',
      '</div>',
      '<div id="m393ShotEditHint" class="m393ShotEditHint">Tap saved holes to select them. Pan and pinch zoom still work.</div>'
    ].join("");
    document.body.appendChild(bar);

    byId("m393ShotEditDone").addEventListener("click", finishShotEditMode);
    byId("m393ShotModeHole").addEventListener("click", function () { shotSetSelectionMode("hole"); });
    byId("m393ShotModeRow").addEventListener("click", function () { shotSetSelectionMode("row"); });
    byId("m393ShotModeColumn").addEventListener("click", function () { shotSetSelectionMode("column"); });
    byId("m393ShotSelectAll").addEventListener("click", shotSelectAllCurrentPage);
    byId("m393ShotCopy").addEventListener("click", function () { shotBuildClipboard("copy"); });
    byId("m393ShotCut").addEventListener("click", function () { shotBuildClipboard("cut"); });
    byId("m393ShotPaste").addEventListener("click", shotArmPaste);
    byId("m393ShotClearSelection").addEventListener("click", function () {
      shotEditSelection = {};
      shotEditPasteArmed = false;
      draw();
      shotEditSetHint("Selection cleared.");
    });
    byId("m393ShotUndo").addEventListener("click", shotUndoLastEdit);
    var arrows = bar.querySelectorAll("[data-m393-edit-shift]");
    for (var i = 0; i < arrows.length; i += 1) {
      arrows[i].addEventListener("click", function () { shotShiftSelection(this.getAttribute("data-m393-edit-shift")); });
    }
    return bar;
  }
  ...
  function patchShotEditDrawing() {
    if (window.__mithrilM393ShotEditDrawing) return;
    window.__mithrilM393ShotEditDrawing = true;
    var originalDraw = window.draw;
  ...
  function installShotEditInteraction(canvas) {
    if (!canvas || canvas.getAttribute("data-m393-shot-edit") === "true") return;
    canvas.setAttribute("data-m393-shot-edit", "true");
  ...
  // ---------------------------------------------------------------------------
  // m39.3 summary calculations
  // ---------------------------------------------------------------------------
  var M393_DEFAULT_HOLE_DIAMETER = 3.5;
  var M393_HOLE_DIAMETERS = [3.0, 3.5, 4.0, 4.5, 5.0, 5.5];
  var M393_ANFO_LB_PER_FT = {
    "3.0": 2.61,
    "3.5": 3.55,
    "4.0": 4.64,
    "4.5": 5.87,
  ...
  function m393NormalizeHoleDiameter(value) {
    var number = Number(value);
    for (var i = 0; i < M393_HOLE_DIAMETERS.length; i += 1) {
      if (Math.abs(number - M393_HOLE_DIAMETERS[i]) < 0.0001) return M393_HOLE_DIAMETERS[i];
    }
    return M393_DEFAULT_HOLE_DIAMETER;
  }

  function m393FormatHoleDiameter(value) {
    return m393NormalizeHoleDiameter(value).toFixed(1) + " in";
  }

  function m393AnfoRate(value) {
    var key = m393NormalizeHoleDiameter(value).toFixed(1);
    return M393_ANFO_LB_PER_FT[key] || M393_ANFO_LB_PER_FT["3.5"];
  }

  function m393StrictPositiveNumber(value) {
  ...
  function m393FormatNumber(value, decimals) {
  ...
  function m393DepthRangeFromValues(values) {
    var valid = [];
    for (var i = 0; i < (values || []).length; i += 1) {
      var parsed = m393StrictPositiveNumber(values[i]);
    ...
    if (Math.abs(minimum - maximum) < 0.0000001) {
      return { count: valid.length, min: minimum, max: maximum, label: "Drilled Depth", value: m393FormatNumber(minimum, 2) + " ft" };
    }
    return {
      count: valid.length,
      min: minimum,
      ...
      value: m393FormatNumber(minimum, 2) + "–" + m393FormatNumber(maximum, 2) + " ft"
    };
  }

  function m393ParseLoad(value, anfoRate) {
    var text = String(value == null ? "" : value).trim();
  ...
  function m393ShotLoadSummary(rows, holeDiameter) {
    var rate = m393AnfoRate(holeDiameter);
    var candidates = [];
    var invalid = [];
    var pageSet = {};
    var i;
    ...
    for (i = 0; i < (rows || []).length; i += 1) {
      ...
      var primary = m393ParseLoad(primaryText, rate);
      var secondary = m393ParseLoad(secondaryText, rate);
      var label = m393PageHoleLabel(row, multiPage);
    ...
    return {
      rate: rate,
      lightest: { weight: minimum, labels: lightLabels, text: m393FormatTiedLabels(lightLabels) + " — " + m393FormatNumber(minimum, 2) + " lb" },
      heaviest: { weight: maximum, labels: heavyLabels, text: m393FormatTiedLabels(heavyLabels) + " — " + m393FormatNumber(maximum, 2) + " lb" },
      invalid: invalid,
      candidates: candidates
    };
  }

  function m393EscapeHTML(value) {
  ...
  function m393DiameterOptions(selectedValue) {
    var selected = m393NormalizeHoleDiameter(selectedValue);
    return M393_HOLE_DIAMETERS.map(function (diameter) {
      var value = diameter.toFixed(1);
      return '<option value="' + value + '"' + (Math.abs(selected - diameter) < 0.0001 ? ' selected' : '') + '>' + value + ' in</option>';
    }).join("");
  }

  function m393EnsureDiameterField(modalId, selectId) {
    var modal = byId(modalId);
    if (!modal || byId(selectId)) return byId(selectId);
    var form = modal.querySelector(".formGrid");
    if (!form) return null;
    var label = document.createElement("label");
    label.className = "m393HoleDiameterField";
    label.textContent = "Hole Diameter";
    var select = document.createElement("select");
    select.id = selectId;
    select.innerHTML = m393DiameterOptions(M393_DEFAULT_HOLE_DIAMETER);
    select.value = m393NormalizeHoleDiameter(M393_DEFAULT_HOLE_DIAMETER).toFixed(1);
    label.appendChild(select);
    form.appendChild(label);
    return select;
  }

  function m393EnsureHeaderDiameter() {
    if (typeof headerData === "undefined" || !headerData) return M393_DEFAULT_HOLE_DIAMETER;
    headerData.HoleDiameter = m393NormalizeHoleDiameter(headerData.HoleDiameter);
    return headerData.HoleDiameter;
  }

  function installDrillSummaryCalculations() {
    if (window.__mithrilM393DrillSummaryCalculations) return;
    if (typeof headerData === "undefined" || typeof pagesData === "undefined") return;
    window.__mithrilM393DrillSummaryCalculations = true;

    var select = m393EnsureDiameterField("infoModal", "m393DrillHoleDiameter");
    m393EnsureHeaderDiameter();
    ...
    if (typeof originalOpenInfo === "function") {
      window.openInfo = function () {
        ...
        var field = byId("m393DrillHoleDiameter");
        if (field) field.value = m393NormalizeHoleDiameter(headerData.HoleDiameter).toFixed(1);
        return result;
      };
    }
    ...
    if (typeof originalSaveInfo === "function") {
      window.saveInfo = function () {
        var field = byId("m393DrillHoleDiameter");
        var selected = m393NormalizeHoleDiameter(field ? field.value : headerData.HoleDiameter);
    ...
    if (typeof originalBuildBackupPayload === "function") {
      window.buildBackupPayload = function () {
        ...
        payload.headerData.HoleDiameter = m393NormalizeHoleDiameter(payload.headerData.HoleDiameter);
        return payload;
      };
    }
    ...
    if (typeof originalGetDrillSummary === "function") {
      window.getDrillSummary = function () {
        ...
        summary.depthRange = m393DepthRangeFromValues(depths);
        summary.holeDiameter = m393EnsureHeaderDiameter();
        return summary;
      };
    }
    ...
    if (typeof originalRenderDrillSummaryCanvas === "function") {
      window.renderDrillSummaryCanvas = function () {
        ...
        var summary = typeof window.getDrillSummary === "function" ? window.getDrillSummary() : { depthRange: m393DepthRangeFromValues([]) };
        var x = canvas.getContext("2d");
        x.save();
        x.textBaseline = "top";
        x.fillStyle = "#222";
        x.font = "800 18px Arial";
        x.fillText("Hole Diameter: " + m393FormatHoleDiameter(summary.holeDiameter), 70, 220);
  ...
  function installShotSummaryCalculations() {
    if (window.__mithrilM393ShotSummaryCalculations) return;
    if (typeof headerData === "undefined" || typeof pagesData === "undefined") return;
    window.__mithrilM393ShotSummaryCalculations = true;

    m393EnsureDiameterField("shotInfoModal", "m393ShotHoleDiameter");
    m393EnsureHeaderDiameter();
    ...
    if (typeof originalOpenShotInfo === "function") {
      window.openShotInfo = function () {
        ...
        var field = byId("m393ShotHoleDiameter");
        if (field) field.value = m393NormalizeHoleDiameter(headerData.HoleDiameter).toFixed(1);
        return result;
      };
    }
    ...
    if (typeof originalGetCurrentShotInfoForBackup === "function") {
      window.getCurrentShotInfoForBackup = function () {
        ...
        info.HoleDiameter = m393EnsureHeaderDiameter();
        return info;
      };
    }
    ...
    if (typeof originalNormalizeLoadedHeaderData === "function") {
      window.normalizeLoadedHeaderData = function (payload) {
        ...
        normalized.HoleDiameter = m393NormalizeHoleDiameter(
    ...
    if (typeof originalGetPrintableReportHTML === "function") {
      window.getPrintableReportHTML = function () {
        ...
        var depthRange = m393DepthRangeFromValues(rows.map(function (row) { return row.Depth; }));
        var diameter = m393EnsureHeaderDiameter();
        var loads = m393ShotLoadSummary(rows, diameter);
        var extraRows = [
          ["Hole Diameter", m393FormatHoleDiameter(diameter)],
        ...
        var extraHTML = extraRows.map(function (pair) {
          return "<tr><th>" + m393EscapeHTML(pair[0]) + "</th><td>" + m393EscapeHTML(pair[1]) + "</td></tr>";
        }).join("");
    ...
    if (typeof originalGetSummaryText === "function") {
      window.getSummaryText = function () {
        ...
        var rows = typeof window.getAllHoleRows === "function" ? window.getAllHoleRows() : [];
        var depthRange = m393DepthRangeFromValues(rows.map(function (row) { return row.Depth; }));
        var loads = m393ShotLoadSummary(rows, m393EnsureHeaderDiameter());
        return base + "\nHole Diameter: " + m393FormatHoleDiameter(headerData.HoleDiameter) +
          "\n" + depthRange.label + ": " + depthRange.value +
          "\nLightest Explosive Load: " + (loads.lightest ? loads.lightest.text : "Not available") +
          "\nHeaviest Explosive Load: " + (loads.heaviest ? loads.heaviest.text : "Not available");
      };
  ...
  function initialize() {
    window.MithrilM393Calculations = {
      normalizeHoleDiameter: m393NormalizeHoleDiameter,
      anfoRate: m393AnfoRate,
      parseLoad: m393ParseLoad,
      depthRangeFromValues: m393DepthRangeFromValues,
      shotLoadSummary: m393ShotLoadSummary
    };
