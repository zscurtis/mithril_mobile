const CACHE_NAME = "mithril-mobile-m40-9-6-4-ipad-portrait-zoom-theme-v1";
const APP_SHELL = [
  "./",
  "./index.html",
  "./shot_diagram_m38.html",
  "./shot_diagram_m34.html?v=40.9.6.4",
...
function shouldPatchHTML(requestUrl) {
  ...
  return path.endsWith("/") || path.endsWith("/index.html") || path.endsWith("/shot_diagram_m38.html");
}
function patchHTMLResponse(response, requestUrl) {
  ...
  return response.text().then(html => {
    ...
    const scriptTags = [
      '<script src="./mithril-menu-m397.js?v=40.9.6.4"></script>',
      '<script src="./mithril-core-m400.js?v=40.9.6.4"></script>'
    ].join("");
...
function getNavigationResponse(request, requestUrl) {
  return fetch(request).then(response => { const copy=response.clone(); caches.open(CACHE_NAME).then(cache=>cache.put(request,copy)); return patchHTMLResponse(response,requestUrl); })
    .catch(() => caches.match(request).then(cached => cached || caches.match("./index.html")).then(response => patchHTMLResponse(response,requestUrl)));
}
self.addEventListener("fetch", event => {
  if (event.request.method !== "GET") return;
  const requestUrl = new URL(event.request.url);
  if (requestUrl.pathname.endsWith("/version.json")) {
    event.respondWith(fetch(event.request,{cache:"no-store"}).catch(() => new Response(JSON.stringify({offline:true}),{status:503,headers:{"Content-Type":"application/json"}}))); return;
  }
  if (event.request.mode === "navigate") { event.respondWith(getNavigationResponse(event.request,requestUrl)); return; }
