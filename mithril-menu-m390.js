(function () {
  ...
  var RELEASE_VERSION = "m39.0";
  var RELEASE_LABEL = "multi-hole selection, copy/cut/paste, cross-page selected-hole shifting, structured quarry hole conditions, GPS callouts, multi quick fill, and canvas themes";
  var THEME_STORAGE_KEY = "mithrilCanvasThemeV1";
  var THEME_CLASS_PREFIX = "m390-theme-";
  var THEME_OPTIONS = [
  ...
  function installPrecisionCanvasCoordinates(canvas, type) {
    if (!canvas || canvas.getAttribute("data-m390-precision-coordinates") === "true") return;
    ...
    canvas.setAttribute("data-m390-precision-coordinates", "true");
  }

  function enableWheelZoom(canvas) {
    if (!canvas || canvas.getAttribute("data-m390-wheel-zoom") === "true") return;
    canvas.setAttribute("data-m390-wheel-zoom", "true");
  ...
  function addShotInfoBackButton() {
    ...
    if (!modal || byId("m390ShotInfoBack")) return;
    var grid = modal.querySelector(".buttonGrid");
    if (!grid) return;

    var button = document.createElement("button");
    button.id = "m390ShotInfoBack";
    button.type = "button";
    button.className = "m390BackMenu";
  ...
  function installShotFrameBridge() {
    ...
    if (!frame || frame.getAttribute("data-m390-bridge") === "true") return;
    frame.setAttribute("data-m390-bridge", "true");

    function injectChildScript() {
      try {
        var childDocument = frame.contentDocument;
        if (!childDocument || !childDocument.documentElement) return false;
        if (childDocument.getElementById("mithrilMenuM390ChildLoader")) return true;

        var script = childDocument.createElement("script");
        script.id = "mithrilMenuM390ChildLoader";
        script.src = "./mithril-menu-m390.js?v=39.0-frame";
  ...
  function injectStyles() {
    if (byId("mithrilMenuM390Styles")) return;

    var style = document.createElement("style");
    style.id = "mithrilMenuM390Styles";
    style.textContent = [
      ".m390MenuIntro{margin:0 0 10px;color:#4b4b4b;font-size:13px;font-weight:750;line-height:1.35}",
      ".m390MenuStack{display:grid;grid-template-columns:1fr;gap:8px}",
      ".m390MenuStack>button{width:100%;min-height:52px;text-align:left;padding:10px 13px;font-size:16px}",
      ".m390MenuStack>button.m390Home{text-align:center}",
      ".m390Section{display:none;margin-top:9px;padding:10px;border:1px solid #bcbcbc;border-radius:11px;background:#f8f8f8}",
      ".m390Section.show{display:block}",
      ".m390SectionTitle{margin:0 0 8px;font-size:16px;font-weight:950}",
      ".m390SectionHelp{margin:0 0 9px;color:#555;font-size:12px;font-weight:750;line-height:1.35}",
      ".m390ActionGrid{display:grid;grid-template-columns:1fr 1fr;gap:8px}",
      ".m390ActionGrid button{min-height:49px}",
      ".m390ActionGrid .wide{grid-column:1/-1}",
      ".m390Subpanel{display:none;grid-column:1/-1;padding:9px;border:1px solid #c7c7c7;border-radius:10px;background:white}",
      ".m390Subpanel.show{display:block}",
      ".m390DirectionGrid{display:grid;grid-template-columns:1fr 1fr 1fr;grid-template-areas:'. up .' 'left center right' '. down .';gap:8px}",
      ".m390DirectionGrid button{min-height:50px;padding:7px 5px}",
      ".m390Up{grid-area:up}.m390Left{grid-area:left}.m390Center{grid-area:center}.m390Right{grid-area:right}.m390Down{grid-area:down}",
      ".m390Spacer{visibility:hidden;pointer-events:none}",
      ".m390DangerZone{margin-top:10px;padding-top:10px;border-top:1px solid #d6aaaa}",
      ".m390DangerZone button{width:100%;min-height:50px}",
      ".m390BackMenu{grid-column:1/-1;background:#eef4ff;border-color:#7aa2d8}",
      ".m390ThemePanel{max-height:46vh;overflow:auto;padding-right:2px}",
      ".m390ThemeGroupTitle{margin:10px 0 6px;font-size:12px;font-weight:950;color:#555;text-transform:uppercase;letter-spacing:.04em}",
      ".m390ThemeGrid{display:grid;grid-template-columns:1fr 1fr;gap:8px}",
      ".m390ThemeButton{min-height:44px;font-size:13px;line-height:1.25;text-align:left}",
      ".m390ThemeButton.active{background:#1f6feb;color:#fff;border-color:#1f6feb}",
      "html.m390-theme-gray,body.m390-theme-gray{background:#2e2e2e !important}",
      "html.m390-theme-dark-slate,body.m390-theme-dark-slate{background-color:#232a31 !important;background-image:radial-gradient(circle at 18% 18%, rgba(255,255,255,.06) 0 3px, transparent 4px),radial-gradient(circle at 76% 70%, rgba(0,0,0,.2) 0 18px, transparent 20px),linear-gradient(135deg,#20262d 0%,#313b46 100%) !important;background-size:140px 140px,220px 220px,cover !important;background-attachment:fixed !important}",
      "html.m390-theme-blue-steel,body.m390-theme-blue-steel{background-color:#566575 !important;background-image:radial-gradient(circle at 20% 20%, rgba(255,255,255,.08) 0 2px, transparent 3px),linear-gradient(135deg,rgba(255,255,255,.08),rgba(255,255,255,0) 38%),linear-gradient(135deg,#4b5a68 0%,#6b7d8f 100%) !important;background-size:150px 150px,cover,cover !important;background-attachment:fixed !important}",
      "html.m390-theme-subtle-grid,body.m390-theme-subtle-grid{background-color:#252e38 !important;background-image:radial-gradient(circle at center, rgba(255,255,255,.03) 1px, transparent 1px),repeating-linear-gradient(0deg, rgba(255,255,255,.06) 0 1px, transparent 1px 26px),repeating-linear-gradient(90deg, rgba(255,255,255,.06) 0 1px, transparent 1px 26px),linear-gradient(135deg,#232b34,#2e3945) !important;background-size:26px 26px,26px 26px,26px 26px,cover !important;background-attachment:fixed !important}",
      "html.m390-theme-gradient-slate,body.m390-theme-gradient-slate{background:#54606f !important;background-image:linear-gradient(135deg,#6b7786 0%,#3c4653 100%) !important;background-attachment:fixed !important}",
      "html.m390-theme-dark-paper,body.m390-theme-dark-paper{background-color:#35383d !important;background-image:radial-gradient(circle at 25% 25%, rgba(255,255,255,.05) 0 2px, transparent 3px),radial-gradient(circle at 75% 60%, rgba(255,255,255,.03) 0 1px, transparent 2px),linear-gradient(135deg,#2c3035 0%,#44484f 100%) !important;background-size:120px 120px,90px 90px,cover !important;background-attachment:fixed !important}",
      "html.m390-theme-soft-quarry-tan,body.m390-theme-soft-quarry-tan{background-color:#b9aea0 !important;background-image:radial-gradient(circle at 20% 20%, rgba(255,255,255,.12) 0 2px, transparent 3px),radial-gradient(circle at 80% 70%, rgba(0,0,0,.08) 0 2px, transparent 3px),linear-gradient(135deg,#c8bdae 0%,#a89b8b 100%) !important;background-size:70px 70px,90px 90px,cover !important;background-attachment:fixed !important}",
      "html.m390-theme-blast-ember,body.m390-theme-blast-ember{background-color:#111 !important;background-image:radial-gradient(circle at 15% 78%, rgba(255,110,0,.72) 0 2%, transparent 8%),radial-gradient(circle at 82% 22%, rgba(255,90,0,.48) 0 1.4%, transparent 7%),repeating-linear-gradient(135deg, rgba(255,120,0,.0) 0 18px, rgba(255,110,0,.18) 18px 19px, transparent 19px 34px),linear-gradient(135deg,#090909,#262626) !important;background-attachment:fixed !important}",
      "html.m390-theme-electric-steel,body.m390-theme-electric-steel{background-color:#0e2032 !important;background-image:radial-gradient(circle at 50% 60%, rgba(0,150,255,.34) 0 18%, transparent 42%),repeating-linear-gradient(135deg, rgba(255,255,255,.05) 0 2px, transparent 2px 18px),linear-gradient(135deg,#091521,#27425f) !important;background-attachment:fixed !important}",
      "html.m390-theme-blast-placard,body.m390-theme-blast-placard{background-color:#111 !important;background-image:linear-gradient(45deg, transparent 38%, rgba(255,150,30,.78) 38% 62%, transparent 62%),linear-gradient(-45deg, transparent 38%, rgba(255,150,30,.78) 38% 62%, transparent 62%),radial-gradient(circle at 80% 25%, rgba(255,165,0,.22) 0 12%, transparent 26%),repeating-linear-gradient(135deg, rgba(255,150,30,.22) 0 12px, transparent 12px 36px),linear-gradient(135deg,#090909,#1b1b1b) !important;background-size:280px 280px,280px 280px,cover,cover,cover !important;background-attachment:fixed !important}",
      "html.m390-theme-copper-quarry,body.m390-theme-copper-quarry{background-color:#5a2b11 !important;background-image:radial-gradient(circle at 25% 30%, rgba(255,170,100,.28) 0 14%, transparent 26%),radial-gradient(circle at 72% 68%, rgba(255,210,130,.16) 0 10%, transparent 24%),repeating-linear-gradient(45deg, rgba(255,255,255,.02) 0 12px, rgba(0,0,0,.08) 12px 22px),linear-gradient(135deg,#4c200c,#8a481f) !important;background-attachment:fixed !important}",
      "html.m390-theme-cobalt-topo,body.m390-theme-cobalt-topo{background-color:#041c3a !important;background-image:radial-gradient(circle at 18% 72%, transparent 0 40px, rgba(0,180,255,.24) 41px 42px, transparent 43px 54px, rgba(0,180,255,.18) 55px 56px, transparent 57px),radial-gradient(circle at 78% 62%, transparent 0 50px, rgba(0,180,255,.22) 51px 52px, transparent 53px 66px, rgba(0,180,255,.18) 67px 68px, transparent 69px),radial-gradient(circle at 56% 22%, transparent 0 32px, rgba(0,180,255,.18) 33px 34px, transparent 35px 48px, rgba(0,180,255,.15) 49px 50px, transparent 51px),linear-gradient(135deg,#031325,#09447a) !important;background-attachment:fixed !important}",
      "html.m390-theme-signal-red-slate,body.m390-theme-signal-red-slate{background-color:#120b0b !important;background-image:radial-gradient(circle at 18% 78%, rgba(255,70,40,.34) 0 18%, transparent 30%),radial-gradient(circle at 84% 20%, rgba(255,60,30,.28) 0 12%, transparent 24%),repeating-linear-gradient(90deg, rgba(255,60,30,.16) 0 2px, transparent 2px 48px),linear-gradient(135deg,#0b0b0b,#272222) !important;background-attachment:fixed !important}",
      "@media(max-width:520px){.m390QuickButton{font-size:0}.m390QuickButton:after{content:'Quick';font-size:14px}.m390FitButton{font-size:0}.m390FitButton:after{content:'Fit';font-size:14px}.m390ActionGrid{grid-template-columns:1fr}.m390ActionGrid .wide{grid-column:auto}.m390DirectionGrid{grid-template-columns:1fr 1fr 1fr}.m390ThemeGrid{grid-template-columns:1fr}}"
    ].join("");
  ...
  function updateToolbar(isShot) {
    ...
    for (var i = 0; i < topButtons.length; i += 1) {
      ...
      if (label === "Quick") {
        ...
        topButtons[i].classList.add("m390QuickButton");
      }
    }
    ...
    if (zoomButtons.length && String(zoomButtons[0].textContent || "").trim() === "Fit") {
      ...
      zoomButtons[0].classList.add("m390FitButton");
    }
  ...
  function hideAllSections(box, exceptId) {
    var sections = box.querySelectorAll(".m390Section");
    var buttons = box.querySelectorAll("[data-m390-section]");
    var i;
    ...
    for (i = 0; i < buttons.length; i += 1) {
      var target = buttons[i].getAttribute("data-m390-section");
      if (target !== exceptId) setButtonArrow(buttons[i], false);
    }
  }

  function hideSubpanels(section, exceptId) {
    var panels = section.querySelectorAll(".m390Subpanel");
    var buttons = section.querySelectorAll("[data-m390-subpanel]");
    var i;
    ...
    for (i = 0; i < buttons.length; i += 1) {
      var target = buttons[i].getAttribute("data-m390-subpanel");
      if (target !== exceptId) setButtonArrow(buttons[i], false);
    }
  }
  ...
  function wireExpandableSections(box) {
    box.addEventListener("click", function (event) {
      var sectionButton = event.target.closest("[data-m390-section]");
      if (sectionButton && box.contains(sectionButton)) {
        event.preventDefault();
        var sectionId = sectionButton.getAttribute("data-m390-section");
      ...
      var subButton = event.target.closest("[data-m390-subpanel]");
      if (subButton && box.contains(subButton)) {
        event.preventDefault();
        var subId = subButton.getAttribute("data-m390-subpanel");
        var subpanel = byId(subId);
        if (!subpanel) return;
        var parentSection = subButton.closest(".m390Section");
  ...
  function resetMenuPanelsWhenClosed(menu) {
    var observer = new MutationObserver(function () {
      ...
      var sections = menu.querySelectorAll(".m390Section,.m390Subpanel");
      var buttons = menu.querySelectorAll("[data-m390-section],[data-m390-subpanel]");
      var i;
  ...
  function refreshThemeButtons(root) {
    ...
    var buttons = root.querySelectorAll ? root.querySelectorAll('[data-m390-theme-choice]') : [];
    for (var i = 0; i < buttons.length; i += 1) {
      var button = buttons[i];
      var label = button.getAttribute('data-label') || button.textContent.replace(/^✓\s*/, '').trim();
      button.setAttribute('data-label', label);
      var active = button.getAttribute('data-m390-theme-choice') === current;
  ...
  function buildThemePickerHtml(panelId) {
    function buildThemeButtons(groupKey) {
      ...
      for (var i = 0; i < THEME_OPTIONS.length; i += 1) {
        ...
        html += '<button type="button" class="m390ThemeButton" data-m390-theme-choice="' + THEME_OPTIONS[i].key + '" data-label="' + THEME_OPTIONS[i].label + '">' + THEME_OPTIONS[i].label + '</button>';
      }
      return html;
    }

    return [
      '<button type="button" class="wide" data-m390-subpanel="' + panelId + '" data-label="Canvas Background" aria-expanded="false">Canvas Background  ›</button>',
      '<div id="' + panelId + '" class="m390Subpanel">',
      '  <div class="m390ThemePanel">',
      '    <p class="m390SectionHelp">Pick a canvas background. The theme applies immediately and stays saved on this device.</p>',
      '    <div class="m390ThemeGroupTitle">Classic Themes</div>',
      '    <div class="m390ThemeGrid">' + buildThemeButtons('classic') + '</div>',
      '    <div class="m390ThemeGroupTitle">Bold Themes</div>',
      '    <div class="m390ThemeGrid">' + buildThemeButtons('bold') + '</div>',
      '    <div class="m390ThemeGroupTitle">Default</div>',
      '    <div class="m390ThemeGrid">' + buildThemeButtons('reset') + '</div>',
      '  </div>',
      '</div>'
    ].join('');
  }
  ...
  function wireThemeButtons(root) {
    ...
    for (var i = 0; i < buttons.length; i += 1) {
      buttons[i].addEventListener('click', function () {
        chooseTheme(this.getAttribute('data-m390-theme-choice'));
      });
    }
  ...
  function patchDrillMenu() {
    var menu = byId("menuModal");
    if (!menu || menu.getAttribute("data-m390-patched") === "drill") return;
    var box = menu.querySelector(".box");
    if (!box) return;

    menu.setAttribute("data-m390-patched", "drill");
    box.innerHTML = [
      '<div class="boxHead"><span>Drill Log Menu</span><button type="button" data-m390-action="close">Close</button></div>',
      '<p class="m390MenuIntro">Daily tools stay visible. Setup, exports, and recovery tools open only when needed.</p>',
      '<div class="m390MenuStack">',
      '  <button type="button" data-m390-action="info">Drill Log Info</button>',
      '  <button type="button" data-m390-section="m390DrillPages" data-label="Page Tools" aria-expanded="false">Page Tools  ›</button>',
      '  <div id="m390DrillPages" class="m390Section">',
      '    <div class="m390SectionTitle">Page Tools</div>',
      '    <div class="m390ActionGrid">',
      '      <button type="button" class="wide" data-m390-subpanel="m390DrillAdd" data-label="Add Page" aria-expanded="false">Add Page  ›</button>',
      '      <div id="m390DrillAdd" class="m390Subpanel">',
      '        <p class="m390SectionHelp">Add a blank page beside the current page.</p>',
      '        <div class="m390DirectionGrid">',
      '          <button type="button" class="m390Up" data-m390-add="up">↑ Add Above</button>',
      '          <button type="button" class="m390Left" data-m390-add="left">← Add Left</button>',
      '          <button type="button" class="m390Center m390Spacer" tabindex="-1" aria-hidden="true">Current</button>',
      '          <button type="button" class="m390Right" data-m390-add="right">Add Right →</button>',
      '          <button type="button" class="m390Down" data-m390-add="down">↓ Add Below</button>',
      '        </div>',
      '      </div>',
      '      <button type="button" data-m390-action="fitAll">Fit All Pages</button>',
      '      <button type="button" class="danger" data-m390-action="deletePage">Delete Current Page</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m390-section="m390DrillExport" data-label="Export & Share" aria-expanded="false">Export &amp; Share  ›</button>',
      '  <div id="m390DrillExport" class="m390Section">',
      '    <div class="m390SectionTitle">Export &amp; Share</div>',
      '    <div class="m390ActionGrid">',
      '      <button type="button" class="primary wide" data-m390-action="finish">Finish &amp; Send to Blaster</button>',
      '      <button type="button" data-m390-action="pdf">Download PDF</button>',
      '      <button type="button" data-m390-action="csv">Export CSV</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m390-section="m390DrillBackup" data-label="Backup & Restore" aria-expanded="false">Backup &amp; Restore  ›</button>',
      '  <div id="m390DrillBackup" class="m390Section">',
      '    <div class="m390SectionTitle">Backup &amp; Restore</div>',
      '    <p class="m390SectionHelp">Download a recovery copy or restore a previously saved Drill Log.</p>',
      '    <div class="m390ActionGrid">',
      '      <button type="button" data-m390-action="backup">Download Backup</button>',
      '      <button type="button" data-m390-action="restore">Restore Backup</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m390-section="m390DrillSettings" data-label="Settings" aria-expanded="false">Settings  ›</button>',
      '  <div id="m390DrillSettings" class="m390Section">',
      '    <div class="m390SectionTitle">Settings</div>',
      '    <div class="m390ActionGrid">',
      '      <button type="button" class="wide" data-m390-action="calibrate">Calibrate Employee / Job</button>',
      buildThemePickerHtml("m390DrillTheme"),
      '      <button id="mithrilUpdateMenuButton" type="button" class="wide" data-m390-action="updates">Check for Updates</button>',
      '    </div>',
      '    <div class="m390DangerZone"><button type="button" class="danger" data-m390-action="clear">Clear Drill Log Data</button></div>',
      '  </div>',
      '  <button id="mithrilHomeMenuButton" type="button" class="m390Home" data-m390-action="home">MITHRIL Home</button>',
      '</div>',
    ...
    wireAction(box, '[data-m390-action="close"]', closeMenu);
    wireAction(box, '[data-m390-action="info"]', function () { runAndClose("openInfo"); });
    wireAction(box, '[data-m390-action="fitAll"]', function () { runAndClose("fitAllPages"); });
    wireAction(box, '[data-m390-action="deletePage"]', function () { runAndClose("deletePage"); });
    wireAction(box, '[data-m390-action="finish"]', function () { runAndClose("finishAndSendToBlaster"); });
    wireAction(box, '[data-m390-action="pdf"]', function () { runAndClose("downloadPDF"); });
    wireAction(box, '[data-m390-action="csv"]', function () { runAndClose("exportCSV"); });
    wireAction(box, '[data-m390-action="backup"]', function () { runAndClose("downloadJSON"); });
    wireAction(box, '[data-m390-action="restore"]', function () {
      closeMenu();
      var input = byId("jsonInput");
      if (input) input.click();
    });
    wireAction(box, '[data-m390-action="calibrate"]', function () { runAndClose("startHeaderCalibration"); });
    wireAction(box, '[data-m390-action="updates"]', function () { runAndClose("checkUpdatesFromDrillLog"); });
    wireAction(box, '[data-m390-action="clear"]', function () { callGlobal("clearAll"); });
    wireAction(box, '[data-m390-action="home"]', function () { runAndClose("returnToSelector"); });
    wireThemeButtons(box);

    var addButtons = box.querySelectorAll("[data-m390-add]");
    for (var i = 0; i < addButtons.length; i += 1) {
      addButtons[i].addEventListener("click", function () {
        runAndClose("addPageAtDirection", [this.getAttribute("data-m390-add")]);
      });
    }
  }
  ...
  function patchShotMenu() {
    var menu = byId("menuModal");
    if (!menu || menu.getAttribute("data-m390-patched") === "shot") return;
    var box = menu.querySelector(".box");
    if (!box) return;

    menu.setAttribute("data-m390-patched", "shot");
    box.innerHTML = [
      '<div class="boxHead"><span>Shot Diagram Menu</span><button type="button" data-m390-action="close">Close</button></div>',
      '<p class="m390MenuIntro">Daily tools stay visible. Page layout, exports, backups, and setup tools open only when needed.</p>',
      '<div class="m390MenuStack">',
      '  <button type="button" data-m390-action="info">Shot Info</button>',
      '  <button type="button" class="primary" data-m390-action="editHoles">Edit Holes</button>',
      '  <button type="button" data-m390-section="m390ShotPages" data-label="Page Tools" aria-expanded="false">Page Tools  ›</button>',
      '  <div id="m390ShotPages" class="m390Section">',
      '    <div class="m390SectionTitle">Page Tools</div>',
      '    <div class="m390ActionGrid">',
      '      <button type="button" class="wide" data-m390-subpanel="m390ShotAdd" data-label="Add Page" aria-expanded="false">Add Page  ›</button>',
      '      <div id="m390ShotAdd" class="m390Subpanel">',
      '        <p class="m390SectionHelp">Add a blank page beside the current page.</p>',
      '        <div class="m390DirectionGrid">',
      '          <button type="button" class="m390Up" data-m390-add="up">↑ Add Above</button>',
      '          <button type="button" class="m390Left" data-m390-add="left">← Add Left</button>',
      ...
      '    </div>',
      '  </div>',
      '  <button type="button" data-m390-section="m390ShotExport" data-label="Export & Share" aria-expanded="false">Export &amp; Share  ›</button>',
      '  <div id="m390ShotExport" class="m390Section">',
      '    <div class="m390SectionTitle">Export &amp; Share</div>',
      '    <div class="m390ActionGrid">',
      '      <button type="button" class="primary wide" data-m390-action="finish">Finish &amp; Export PDF</button>',
      '      <button type="button" data-m390-action="shareCsv">Share CSV</button>',
      '      <button type="button" data-m390-action="csv">Download CSV</button>',
      '      <button type="button" class="wide" data-m390-action="pdf">Download PDF</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m390-section="m390ShotBackup" data-label="Backup & Restore" aria-expanded="false">Backup &amp; Restore  ›</button>',
      '  <div id="m390ShotBackup" class="m390Section">',
      '    <div class="m390SectionTitle">Backup &amp; Restore</div>',
      '    <p class="m390SectionHelp">Download a recovery copy or restore a previously saved Shot Diagram.</p>',
      '    <div class="m390ActionGrid">',
      '      <button type="button" data-m390-action="backup">Download Backup</button>',
      '      <button type="button" data-m390-action="restore">Restore Backup</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m390-section="m390ShotSettings" data-label="Settings" aria-expanded="false">Settings  ›</button>',
      '  <div id="m390ShotSettings" class="m390Section">',
      '    <div class="m390SectionTitle">Settings</div>',
      '    <div class="m390ActionGrid">',
      '      <button type="button" class="wide" data-m390-action="calibrate">Field Calibration</button>',
      buildThemePickerHtml("m390ShotTheme"),
      '      <button id="mithrilUpdateMenuButton" type="button" class="wide" data-m390-action="updates">Check for Updates</button>',
      '    </div>',
      '    <div class="m390DangerZone"><button type="button" class="danger" data-m390-action="clear">Clear Shot Data</button></div>',
      '  </div>',
      '  <button id="mithrilHomeMenuButton" type="button" class="m390Home" data-m390-action="home">MITHRIL Home</button>',
      '</div>',
    ...
    wireAction(box, '[data-m390-action="close"]', closeMenu);
    wireAction(box, '[data-m390-action="info"]', function () { runAndClose("openShotInfo"); });
    wireAction(box, '[data-m390-action="editHoles"]', function () { closeMenu(); startShotEditMode(); });
    wireAction(box, '[data-m390-action="fitAll"]', function () { runAndClose("fitAllPages"); });
    wireAction(box, '[data-m390-action="deletePage"]', function () { runAndClose("deleteCurrentPage"); });
    wireAction(box, '[data-m390-action="finish"]', function () { runAndClose("finishAndSend"); });
    wireAction(box, '[data-m390-action="shareCsv"]', function () { runAndClose("emailCSV"); });
    wireAction(box, '[data-m390-action="csv"]', function () { runAndClose("exportCSV"); });
    wireAction(box, '[data-m390-action="pdf"]', function () { runAndClose("exportPDFReport"); });
    wireAction(box, '[data-m390-action="backup"]', function () { runAndClose("downloadJSON"); });
    wireAction(box, '[data-m390-action="restore"]', function () { runAndClose("triggerLoadJSON"); });
    wireAction(box, '[data-m390-action="calibrate"]', function () { runAndClose("openFieldCalibration"); });
    wireAction(box, '[data-m390-action="updates"]', checkShotUpdates);
    wireAction(box, '[data-m390-action="clear"]', function () { callGlobal("clearAll"); });
    wireAction(box, '[data-m390-action="home"]', function () { closeMenu(); homeFromShot(); });
    wireThemeButtons(box);
  ...
  function injectMultiQuickStyles() {
    if (byId("mithrilMultiQuickM390Styles")) return;
    var style = document.createElement("style");
    style.id = "mithrilMultiQuickM390Styles";
    style.textContent = [
      ".m390QuickIntro{margin:0 0 12px;color:#444;font-size:13px;font-weight:750;line-height:1.4}",
      ".m390QuickRows{display:grid;gap:10px}",
      ".m390QuickRow{display:grid;grid-template-columns:78px minmax(130px,1.15fr) minmax(110px,.85fr);gap:8px;align-items:end;padding:9px;border:1px solid #bbb;border-radius:10px;background:#f8f8f8}",
      ".m390QuickRow.inactive{opacity:.68}",
      ".m390QuickUse{display:flex;align-items:center;justify-content:center;gap:6px;min-height:46px;padding:6px;border:1px solid #aaa;border-radius:8px;background:#fff;font-size:13px;font-weight:950}",
      ".m390QuickUse input{width:24px;height:24px;min-height:24px;margin:0;padding:0}",
      ".m390QuickRow label{min-width:0}",
      ".m390QuickRow select,.m390QuickRow input{width:100%;min-height:46px;font-size:17px;padding:8px;border:1px solid #999;border-radius:8px;background:#fff}",
      ".m390QuickValueCell{min-width:0}",
      ".m390QuickActions{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:12px}",
      ".m390QuickActions button{min-height:48px}",
      ".m390QuickBarSummary{min-width:0;font-size:14px;font-weight:950;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
      ".m390QuickBarHint{grid-column:1/-1;font-size:12px;font-weight:850;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#333}",
      "#quickBar.m390MultiQuickBar,#singleFillBar.m390MultiQuickBar{grid-template-columns:minmax(0,1fr) auto auto!important}",
      "#quickModal .box.m390QuickModalBox{width:min(650px,96vw)}",
      "#quickModal .box.m390QuickKeypadOpen{padding-bottom:365px!important}",
      "@media(max-width:520px){.m390QuickRow{grid-template-columns:68px 1fr}.m390QuickValueCell{grid-column:2}.m390QuickActions{grid-template-columns:1fr}.m390QuickBarSummary{font-size:13px}}"
    ].join("");
    document.head.appendChild(style);
  }
  ...
  function buildMultiQuickRows(prefix, fields) {
    ...
    for (var i = 1; i <= 3; i += 1) {
      html += [
        '<div id="' + prefix + 'Row' + i + '" class="m390QuickRow">',
        '  <label class="m390QuickUse"><input id="' + prefix + 'Use' + i + '" type="checkbox" /> Use ' + i + '</label>',
        '  <label>Field<select id="' + prefix + 'Field' + i + '">' + options + '</select></label>',
        '  <label class="m390QuickValueCell">Value',
  ...
  function installCanvasBackgroundBridge(canvas) {
    if (!canvas || canvas.getAttribute("data-m390-theme-canvas") === "true") return;
    var context = canvas.getContext && canvas.getContext("2d");
    if (!context || context.__mithrilM390FillRect) return;
    var originalFillRect = context.fillRect.bind(context);
    context.__mithrilM390FillRect = originalFillRect;
    ...
    canvas.setAttribute("data-m390-theme-canvas", "true");
  }
  ...
  function drillQuickPrefix() { return "m390DrillQuick"; }
  ...
  function updateDrillMultiQuickBar(message) {
    ...
    var summary = byId("m390DrillQuickSummary");
    var hint = byId("m390DrillQuickHint");
  ...
  function installDrillQuickPadSupport() {
    if (window.__mithrilM390DrillPad) return;
    window.__mithrilM390DrillPad = true;
    ...
    window.showPad = function (id) {
      ...
      if (box && byId("quickModal").classList.contains("show")) box.classList.add("m390QuickKeypadOpen");
    ...
    window.hidePad = function () {
      ...
      var box = byId("quickModal") && byId("quickModal").querySelector(".box");
      if (box) box.classList.remove("m390QuickKeypadOpen");
    };
  ...
  function patchDrillMultiQuick() {
    ...
    var bar = byId("quickBar");
    if (!modal || !bar || modal.getAttribute("data-m390-multi-quick") === "drill") return;
    modal.setAttribute("data-m390-multi-quick", "drill");
    var box = modal.querySelector(".box");
    if (!box) return;
    box.classList.add("m390QuickModalBox");
    box.innerHTML = [
      '<div class="boxHead"><span>Quick Fill</span><button type="button" id="m390DrillQuickClose">Close</button></div>',
      '<p class="m390QuickIntro">Use up to three different fields. One tap on a hole applies every active row together.</p>',
      '<div class="m390QuickRows">' + buildMultiQuickRows(drillQuickPrefix(), DRILL_MULTI_QUICK_FIELDS) + '</div>',
      '<div class="m390QuickActions">',
      '  <button type="button" class="primary" id="m390DrillQuickOn">Turn On</button>',
      '  <button type="button" id="m390DrillQuickOff">Turn Off</button>',
      '  <button type="button" class="danger" id="m390DrillQuickClear">Clear Values</button>',
      '</div>'
    ].join("");

    bar.classList.add("m390MultiQuickBar");
    bar.innerHTML = [
      '<div id="m390DrillQuickSummary" class="m390QuickBarSummary"></div>',
      '<button type="button" id="m390DrillQuickEdit">Edit</button>',
      '<button type="button" class="danger" id="m390DrillQuickBarOff">Off</button>',
      '<div id="m390DrillQuickHint" class="m390QuickBarHint"></div>'
    ].join("");
    ...
    byId("m390DrillQuickClose").addEventListener("click", function () { window.closeQuickModal(); });
    byId("m390DrillQuickOn").addEventListener("click", function () { saveDrillMultiQuick(true); });
    byId("m390DrillQuickOff").addEventListener("click", function () { saveDrillMultiQuick(false); });
    byId("m390DrillQuickClear").addEventListener("click", function () {
      for (var i = 1; i <= 3; i += 1) {
    ...
    byId("m390DrillQuickEdit").addEventListener("click", function () { window.openQuickModal(); });
    byId("m390DrillQuickBarOff").addEventListener("click", function () { window.turnQuickOff(); });
  ...
  function shotQuickPrefix() { return "m390ShotQuick"; }
  ...
  function updateShotMultiQuickBar(message) {
    ...
    if (!bar) return;
    bar.classList.toggle("show", !!state.enabled);
    var summary = byId("m390ShotQuickSummary");
    var hint = byId("m390ShotQuickHint");
  ...
  function installShotQuickKeypadSupport() {
    if (window.__mithrilM390ShotPad) return;
    window.__mithrilM390ShotPad = true;
    ...
    window.scrollActiveLoadInputIntoView = function (id) {
      ...
      if (customIndex(id)) {
        var box = byId("quickModal") && byId("quickModal").querySelector(".box");
        if (box) box.classList.add("m390QuickKeypadOpen");
      }
    };
  ...
  function patchShotMultiQuick() {
    var modal = byId("quickModal");
    var bar = byId("singleFillBar");
    if (!modal || !bar || modal.getAttribute("data-m390-multi-quick") === "shot") return;
    modal.setAttribute("data-m390-multi-quick", "shot");
    var box = modal.querySelector(".box");
    if (!box) return;
    box.classList.add("m390QuickModalBox");
    box.innerHTML = [
      '<div class="boxHead"><span>Quick Fill</span><button type="button" id="m390ShotQuickClose">Close</button></div>',
      '<p class="m390QuickIntro">Use up to three different fields. One tap on a hole applies every active row together.</p>',
      '<div class="m390QuickRows">' + buildMultiQuickRows(shotQuickPrefix(), SHOT_MULTI_QUICK_FIELDS) + '</div>',
      '<div class="m390QuickActions">',
      '  <button type="button" class="primary" id="m390ShotQuickOn">Turn On</button>',
      '  <button type="button" id="m390ShotQuickOff">Turn Off</button>',
      '  <button type="button" class="danger" id="m390ShotQuickClear">Clear Values</button>',
      '</div>'
    ].join("");

    bar.classList.add("m390MultiQuickBar");
    bar.innerHTML = [
      '<div id="m390ShotQuickSummary" class="m390QuickBarSummary"></div>',
      '<button type="button" id="m390ShotQuickEdit">Edit</button>',
      '<button type="button" class="danger" id="m390ShotQuickBarOff">Off</button>',
      '<div id="m390ShotQuickHint" class="m390QuickBarHint"></div>'
    ].join("");
    ...
    byId("m390ShotQuickClose").addEventListener("click", function () { window.closeQuickEntry(); });
    byId("m390ShotQuickOn").addEventListener("click", function () { saveShotMultiQuick(true); });
    byId("m390ShotQuickOff").addEventListener("click", function () { saveShotMultiQuick(false); });
    byId("m390ShotQuickClear").addEventListener("click", function () {
      for (var i = 1; i <= 3; i += 1) {
        byId(prefix + "Value" + i).value = "";
        byId(prefix + "Bool" + i).value = "Yes";
      }
    ...
    byId("m390ShotQuickEdit").addEventListener("click", function () { window.openQuickEntry(); });
    byId("m390ShotQuickBarOff").addEventListener("click", function () { window.quickEnabledOff(); });
  ...
  function injectGPSStyles() {
    if (byId("mithrilGPSM390Styles")) return;
    var style = document.createElement("style");
    style.id = "mithrilGPSM390Styles";
    style.textContent = [
      ".m390GPSDetails{margin-top:12px;border:1px solid #9ab5d6;border-radius:10px;background:#f6faff;overflow:hidden}",
      ".m390GPSDetails summary{min-height:48px;display:flex;align-items:center;padding:8px 11px;font-size:15px;font-weight:950;cursor:pointer;user-select:none}",
      ".m390GPSBody{padding:0 10px 10px}",
      ".m390GPSGrid{display:grid;grid-template-columns:1fr 1fr;gap:9px}",
      ".m390GPSGrid label{min-width:0}",
      ".m390GPSGrid input{width:100%;min-height:44px;font-size:17px;padding:8px;border:1px solid #999;border-radius:8px;box-sizing:border-box}",
      ".m390GPSGrid .wide{grid-column:1/-1}",
      ".m390GPSActions{display:grid;grid-template-columns:1fr;gap:8px;margin-top:9px}",
      ".m390GPSActions button{min-height:46px}",
      ".m390GPSStatus{margin-top:8px;min-height:18px;font-size:12px;font-weight:800;color:#365b82;line-height:1.3}",
      ".m390GPSMenuGrid{display:grid;grid-template-columns:1fr 1fr;gap:8px}",
      ".m390GPSMenuGrid button{min-height:48px}",
      ".m390GPSMenuGrid .wide{grid-column:1/-1}",
      ".m390GPSArrangeBar{display:none;position:fixed;left:8px;right:8px;bottom:8px;z-index:240;grid-template-columns:minmax(0,1fr) auto auto;gap:8px;align-items:center;background:rgba(255,255,255,.98);border:2px solid #8a4fff;border-radius:12px;padding:8px;box-shadow:0 5px 18px rgba(0,0,0,.38)}",
      ".m390GPSArrangeBar.show{display:grid}",
      ".m390GPSArrangeHint{font-size:13px;line-height:1.25;font-weight:900;color:#333;min-width:0}",
      ".m390GPSArrangeBar button{min-height:44px}",
      "@media(max-width:520px){.m390GPSGrid{grid-template-columns:1fr}.m390GPSGrid .wide{grid-column:auto}.m390GPSActions{grid-template-columns:1fr}.m390GPSMenuGrid{grid-template-columns:1fr}.m390GPSMenuGrid .wide{grid-column:auto}.m390GPSArrangeBar{grid-template-columns:1fr 1fr}.m390GPSArrangeHint{grid-column:1/-1}}"
    ].join("");
    document.head.appendChild(style);
  }
  ...
  function patchGPSDrawing(type) {
    if (window.__mithrilM390GPSDrawing) return;
    window.__mithrilM390GPSDrawing = true;
  ...
  function gpsEditorPrefix(type) {
    return type === "drill" ? "m390DrillGPS" : "m390ShotGPS";
  }
  ...
  function installGPSHoleEditor(type) {
    ...
    if (!modal || modal.getAttribute("data-m390-gps-editor") === type) return;
    ...
    details.className = "m390GPSDetails";
    details.innerHTML = [
      '<summary>Manual GPS Tag (optional)</summary>',
      '<div class="m390GPSBody">',
      '  <p class="m390GPSStatus">Enter coordinates collected with your external GPS equipment. MITHRIL does not request or capture this device\'s location.</p>',
      '  <div class="m390GPSGrid">',
      ...
      '  </div>',
      '  <div class="m390GPSActions">',
      '    <button type="button" class="danger" id="' + prefix + 'Clear">Clear GPS Tag</button>',
      '  </div>',
      '  <div id="' + prefix + 'Status" class="m390GPSStatus">No manual GPS tag saved for this hole.</div>',
      '</div>'
    ].join("");
    ...
    modal.setAttribute("data-m390-gps-editor", type);
  }
  ...
  function updateGPSMenuState(type) {
    var button = byId(type === "drill" ? "m390DrillGPSToggle" : "m390ShotGPSToggle");
    var count = byId(type === "drill" ? "m390DrillGPSCount" : "m390ShotGPSCount");
  ...
  function ensureGPSArrangeBar(type) {
    var bar = byId("m390GPSArrangeBar");
    if (!bar) {
      bar = document.createElement("div");
      bar.id = "m390GPSArrangeBar";
      bar.className = "m390GPSArrangeBar";
      bar.innerHTML = [
        '<div id="m390GPSArrangeHint" class="m390GPSArrangeHint">Drag GPS boxes to move them. The arrows follow automatically.</div>',
        '<button type="button" id="m390GPSArrangeReset">Reset Page</button>',
        '<button type="button" class="primary" id="m390GPSArrangeDone">Done</button>'
      ].join("");
      document.body.appendChild(bar);
      byId("m390GPSArrangeDone").addEventListener("click", function () { finishGPSArrange(); });
      byId("m390GPSArrangeReset").addEventListener("click", function () { resetGPSPositions(gpsArrangeType || type); });
    }
    return bar;
  }
  ...
  function startGPSArrange(type) {
    ...
    byId("m390GPSArrangeHint").textContent = "Drag GPS boxes on any visible page. Touching a box automatically activates its page.";
    if (typeof window.draw === "function") window.draw();
  }
  ...
  function buildGPSMenuHtml(type) {
    var prefix = type === "drill" ? "m390DrillGPS" : "m390ShotGPS";
    return [
      '<button type="button" class="wide" data-m390-subpanel="' + prefix + 'Panel" data-label="GPS Callouts" aria-expanded="false">GPS Callouts  ›</button>',
      '<div id="' + prefix + 'Panel" class="m390Subpanel">',
      '  <p class="m390SectionHelp">Coordinates entered manually are stored on each hole. Visible callouts and arrows are also included in PDF exports.</p>',
      '  <div id="' + prefix + 'Count" class="m390GPSStatus"></div>',
      '  <div class="m390GPSMenuGrid">',
  ...
  function installGPSMenuTools(type) {
    var settings = byId(type === "drill" ? "m390DrillSettings" : "m390ShotSettings");
    if (!settings || settings.getAttribute("data-m390-gps-menu") === "true") return;
    var grid = settings.querySelector(".m390ActionGrid");
    if (!grid) return;
    ...
    settings.setAttribute("data-m390-gps-menu", "true");
    updateGPSMenuState(type);
    if (!window.__mithrilM390GPSMenuOpenWrapped && typeof window.openMenu === "function") {
      var originalOpenMenuForGPS = window.openMenu;
      window.openMenu = function () {
        updateGPSMenuState(type);
        return originalOpenMenuForGPS.apply(this, arguments);
      };
      window.__mithrilM390GPSMenuOpenWrapped = true;
    }
  }
  ...
  function activateGPSPage(type, pageNum) {
    ...
    try {
      ...
      var hint = byId("m390GPSArrangeHint");
  ...
  function installGPSDragging(canvas, type) {
    if (!canvas || canvas.getAttribute("data-m390-gps-drag") === "true") return;
    ...
    canvas.setAttribute("data-m390-gps-drag", "true");
  }
  ...
  function injectHoleConditionStyles() {
    if (byId("mithrilHoleConditionsM390Styles")) return;
    var style = document.createElement("style");
    style.id = "mithrilHoleConditionsM390Styles";
    style.textContent = [
      ".m390ConditionEditButton{grid-column:1/-1;min-height:46px;background:#fff8c9;border-color:#c8a600;color:#3d3300}",
      ".m390ConditionEditButton.hidden{display:none}",
      "#m390ConditionModal{z-index:260}",
      "#m390ConditionModal .box{width:min(720px,97vw)}",
      ".m390ConditionIntro{margin:0 0 10px;color:#444;font-size:13px;font-weight:750;line-height:1.4}",
      ".m390ConditionRows{display:grid;gap:10px}",
      ".m390ConditionRow{position:relative;display:grid;grid-template-columns:minmax(145px,1.25fr) minmax(100px,.7fr) minmax(120px,.85fr);gap:8px;padding:10px;border:1px solid #c8ad44;border-radius:11px;background:#fffbea}",
      ".m390ConditionRow label{min-width:0}",
      ".m390ConditionRow select,.m390ConditionRow input[type=number],.m390ConditionRow input[type=text]{width:100%;min-height:44px;padding:8px;border:1px solid #999;border-radius:8px;background:#fff;font-size:17px}",
      ".m390ConditionBottom{grid-column:1/3;display:flex;align-items:center;gap:9px;min-height:44px;padding:7px 9px;border:1px solid #aaa;border-radius:8px;background:#fff;font-size:14px;font-weight:900}",
      ".m390ConditionBottom input{width:25px;height:25px;min-height:25px;margin:0}",
      ".m390ConditionRemove{align-self:end;min-height:44px;background:#fff1f1;border-color:#c66}",
      ".m390ConditionOther{grid-column:1/-1}",
      ".m390ConditionOther.hidden{display:none}",
      ".m390ConditionTools{display:grid;grid-template-columns:1fr auto;gap:8px;margin-top:10px}",
      ".m390ConditionPreview{margin-top:10px;padding:10px;border:1px solid #9f8a2d;border-radius:9px;background:#fffdf3;font-size:14px;font-weight:850;line-height:1.4;color:#342d0b}",
      ".m390ConditionActions{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:12px}",
      ".m390ConditionActions button{min-height:48px}",
      "@media(max-width:620px){.m390ConditionRow{grid-template-columns:1fr 1fr}.m390ConditionRow>label:first-child{grid-column:1/-1}.m390ConditionBottom{grid-column:1/-1}.m390ConditionRemove{grid-column:1/-1}.m390ConditionActions{grid-template-columns:1fr}.m390ConditionTools{grid-template-columns:1fr}}"
    ].join("");
    document.head.appendChild(style);
  }

  function ensureHoleConditionModal() {
    var modal = byId("m390ConditionModal");
    if (modal) return modal;
    modal = document.createElement("div");
    modal.id = "m390ConditionModal";
    modal.className = "modal";
    modal.innerHTML = [
      '<div class="box">',
      '  <div class="boxHead"><span id="m390ConditionTitle">Hole Conditions</span><button type="button" id="m390ConditionClose">Close</button></div>',
      '  <p class="m390ConditionIntro">Record where a loading condition begins and where competent rock resumes. These intervals are informational and never reduce Total Rock.</p>',
      '  <div id="m390ConditionRows" class="m390ConditionRows"></div>',
      '  <div class="m390ConditionTools"><button type="button" id="m390ConditionAdd">+ Add another condition</button><button type="button" id="m390ConditionClear" class="danger">Clear all</button></div>',
      '  <div id="m390ConditionPreview" class="m390ConditionPreview">No condition intervals entered.</div>',
      '  <div class="m390ConditionActions"><button type="button" class="primary" id="m390ConditionSave">Save Conditions</button><button type="button" id="m390ConditionCancel">Cancel</button><button type="button" id="m390ConditionDoneHole">Save Conditions + Hole</button></div>',
      '</div>'
    ].join("");
    document.body.appendChild(modal);

    byId("m390ConditionClose").addEventListener("click", cancelHoleConditionModal);
    byId("m390ConditionCancel").addEventListener("click", cancelHoleConditionModal);
    byId("m390ConditionAdd").addEventListener("click", function () {
    ...
    byId("m390ConditionClear").addEventListener("click", function () {
      conditionDraft = [];
      renderHoleConditionRows();
    });
    byId("m390ConditionSave").addEventListener("click", function () { saveHoleConditionModal(false); });
    byId("m390ConditionDoneHole").addEventListener("click", function () { if (saveHoleConditionModal(false)) window.saveHole(false); });
    byId("m390ConditionRows").addEventListener("input", readConditionRowsIntoDraft);
    byId("m390ConditionRows").addEventListener("change", function (event) {
      readConditionRowsIntoDraft();
      if (event.target && event.target.classList.contains("m390ConditionType")) renderHoleConditionRows();
      else updateHoleConditionPreview();
    });
    byId("m390ConditionRows").addEventListener("click", function (event) {
  ...
  function readConditionRowsIntoDraft() {
    var rows = document.querySelectorAll("#m390ConditionRows .m390ConditionRow");
    var next = [];
    for (var i = 0; i < rows.length; i += 1) {
      var row = rows[i];
      var type = row.querySelector(".m390ConditionType");
      var other = row.querySelector(".m390ConditionOtherInput");
      var start = row.querySelector(".m390ConditionStart");
      var end = row.querySelector(".m390ConditionEnd");
      var bottom = row.querySelector(".m390ConditionToBottom");
      next.push({
  ...
  function renderHoleConditionRows() {
    var container = byId("m390ConditionRows");
    if (!container) return;
    var html = "";
    for (var i = 0; i < conditionDraft.length; i += 1) {
      var entry = conditionDraft[i];
      var otherClass = entry.type === "Other" ? "m390ConditionOther" : "m390ConditionOther hidden";
      html += [
        '<div class="m390ConditionRow" data-condition-index="' + i + '">',
        '  <label>Condition<select class="m390ConditionType">' + conditionOptionsHtml(entry.type) + '</select></label>',
        '  <label>Starts at (ft)<input class="m390ConditionStart" type="number" inputmode="decimal" step="0.1" min="0" value="' + String(entry.start || "") + '" /></label>',
        '  <label>Competent rock resumes (ft)<input class="m390ConditionEnd" type="number" inputmode="decimal" step="0.1" min="0" value="' + String(entry.end || "") + '"' + (entry.toBottom ? ' disabled' : '') + ' /></label>',
        '  <label class="m390ConditionBottom"><input class="m390ConditionToBottom" type="checkbox"' + (entry.toBottom ? ' checked' : '') + ' /> Continues to bottom</label>',
        '  <button type="button" class="m390ConditionRemove" data-condition-remove="' + i + '">Remove</button>',
        '  <label class="' + otherClass + '">Other condition name<input class="m390ConditionOtherInput" type="text" value="' + String(entry.other || "").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;") + '" /></label>',
        '</div>'
      ].join("");
    }
    if (!conditionDraft.length) html = '<div class="m390ConditionPreview">No condition intervals. Tap “Add another condition” to begin.</div>';
  ...
  function updateConditionEditorControls() {
    var rows = document.querySelectorAll("#m390ConditionRows .m390ConditionRow");
    for (var i = 0; i < rows.length; i += 1) {
      var bottom = rows[i].querySelector(".m390ConditionToBottom");
      var end = rows[i].querySelector(".m390ConditionEnd");
      var type = rows[i].querySelector(".m390ConditionType");
      var otherWrap = rows[i].querySelector(".m390ConditionOther");
      if (end && bottom) {
  ...
  function updateHoleConditionPreview() {
    var preview = byId("m390ConditionPreview");
    if (!preview) return;
  ...
  function openHoleConditionModal(createDefault) {
    ...
    byId("m390ConditionTitle").textContent = "Hole Conditions — " + (typeof currentHole !== "undefined" ? currentHole : "Hole");
    renderHoleConditionRows();
    byId("m390ConditionModal").classList.add("show");
    window.setTimeout(function () {
      var input = document.querySelector("#m390ConditionRows .m390ConditionStart");
  ...
  function updateHoleConditionButton() {
    var button = byId("m390EditHoleConditions");
    var checkbox = byId("breakthrough");
  ...
  function installDrillHoleConditions() {
    if (window.__mithrilM390HoleConditions) return;
    var checkbox = byId("breakthrough");
    var holeBox = byId("holeBox");
    if (!checkbox || !holeBox) return;
    window.__mithrilM390HoleConditions = true;
    ...
    edit.id = "m390EditHoleConditions";
    edit.type = "button";
    edit.className = "m390ConditionEditButton hidden";
  ...
  function patchDrillConditionCSV() {
    if (window.__mithrilM390ConditionCSV || typeof window.exportCSV !== "function") return;
    window.__mithrilM390ConditionCSV = true;
    window.exportCSV = function () {
  ...
  function shotEditSetHint(message) {
    var hint = byId("m390ShotEditHint");
  ...
  function shotUpdateEditBar() {
    var count = shotEditSelectionList().length;
    var status = byId("m390ShotEditStatus");
    if (status) status.textContent = shotSelectionDescription();
    var modes = ["hole", "row", "column"];
    for (var i = 0; i < modes.length; i += 1) {
      var button = byId("m390ShotMode" + modes[i].charAt(0).toUpperCase() + modes[i].slice(1));
      if (button) button.classList.toggle("active", shotEditSelectMode === modes[i]);
    }
    var hasSelection = count > 0;
    var copyButton = byId("m390ShotCopy");
    var cutButton = byId("m390ShotCut");
    var pasteButton = byId("m390ShotPaste");
    ...
    var arrows = document.querySelectorAll("[data-m390-edit-shift]");
    for (var a = 0; a < arrows.length; a += 1) arrows[a].disabled = !hasSelection;
    var undoButton = byId("m390ShotUndo");
    if (undoButton) undoButton.disabled = !shotEditUndoHistory.length;
  }

  function injectShotEditStyles() {
    if (byId("mithrilShotEditM390Styles")) return;
    var style = document.createElement("style");
    style.id = "mithrilShotEditM390Styles";
    style.textContent = [
      ".m390ShotEditBar{display:none;position:fixed;left:8px;right:8px;bottom:8px;z-index:245;background:rgba(255,255,255,.985);border:2px solid #8a4fff;border-radius:13px;padding:8px;box-shadow:0 6px 22px rgba(0,0,0,.42);gap:7px}",
      ".m390ShotEditBar.show{display:grid}",
      ".m390ShotEditHead{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:center}",
      ".m390ShotEditTitle{font-size:14px;font-weight:950;color:#222}",
      ".m390ShotEditStatus{font-size:12px;font-weight:850;color:#5d3b96;margin-top:2px}",
      ".m390ShotEditModes,.m390ShotEditActions{display:grid;grid-template-columns:repeat(4,1fr);gap:6px}",
      ".m390ShotEditModes button,.m390ShotEditActions button{min-height:43px;padding:5px;font-size:13px}",
      ".m390ShotEditModes button.active{background:#8a4fff;color:#fff;border-color:#6f35da}",
      ".m390ShotEditDirections{display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr;gap:6px}",
      ".m390ShotEditDirections button{min-height:45px;font-size:18px;padding:4px}",
      ".m390ShotEditHint{min-height:17px;font-size:11px;line-height:1.25;font-weight:800;color:#444}",
      ".m390ShotEditDone{min-width:76px;background:#1f6feb;color:#fff;border-color:#1f6feb}",
      "@media(max-width:520px){.m390ShotEditModes,.m390ShotEditActions{grid-template-columns:repeat(2,1fr)}.m390ShotEditDirections{grid-template-columns:repeat(5,1fr)}.m390ShotEditDirections button{font-size:17px}.m390ShotEditHead{grid-template-columns:1fr auto}}"
    ].join("");
    document.head.appendChild(style);
  }

  function ensureShotEditBar() {
    var bar = byId("m390ShotEditBar");
    if (bar) return bar;
    bar = document.createElement("div");
    bar.id = "m390ShotEditBar";
    bar.className = "m390ShotEditBar";
    bar.innerHTML = [
      '<div class="m390ShotEditHead">',
      '  <div><div class="m390ShotEditTitle">Edit Holes</div><div id="m390ShotEditStatus" class="m390ShotEditStatus">0 selected</div></div>',
      '  <button type="button" id="m390ShotEditDone" class="m390ShotEditDone">Done</button>',
      '</div>',
      '<div class="m390ShotEditModes">',
      '  <button type="button" id="m390ShotModeHole">Hole</button>',
      '  <button type="button" id="m390ShotModeRow">Row</button>',
      '  <button type="button" id="m390ShotModeColumn">Column</button>',
      '  <button type="button" id="m390ShotSelectAll">All Page</button>',
      '</div>',
      '<div class="m390ShotEditActions">',
      '  <button type="button" id="m390ShotCopy">Copy</button>',
      '  <button type="button" id="m390ShotCut">Cut</button>',
      '  <button type="button" id="m390ShotPaste">Paste</button>',
      '  <button type="button" id="m390ShotClearSelection">Clear Selection</button>',
      '</div>',
      '<div class="m390ShotEditDirections">',
      '  <button type="button" data-m390-edit-shift="left" aria-label="Shift left">←</button>',
      '  <button type="button" data-m390-edit-shift="up" aria-label="Shift up">↑</button>',
      '  <button type="button" id="m390ShotUndo" aria-label="Undo">↶</button>',
      '  <button type="button" data-m390-edit-shift="down" aria-label="Shift down">↓</button>',
      '  <button type="button" data-m390-edit-shift="right" aria-label="Shift right">→</button>',
      '</div>',
      '<div id="m390ShotEditHint" class="m390ShotEditHint">Tap saved holes to select them. Pan and pinch zoom still work.</div>'
    ].join("");
    document.body.appendChild(bar);

    byId("m390ShotEditDone").addEventListener("click", finishShotEditMode);
    byId("m390ShotModeHole").addEventListener("click", function () { shotSetSelectionMode("hole"); });
    byId("m390ShotModeRow").addEventListener("click", function () { shotSetSelectionMode("row"); });
    byId("m390ShotModeColumn").addEventListener("click", function () { shotSetSelectionMode("column"); });
    byId("m390ShotSelectAll").addEventListener("click", shotSelectAllCurrentPage);
    byId("m390ShotCopy").addEventListener("click", function () { shotBuildClipboard("copy"); });
    byId("m390ShotCut").addEventListener("click", function () { shotBuildClipboard("cut"); });
    byId("m390ShotPaste").addEventListener("click", shotArmPaste);
    byId("m390ShotClearSelection").addEventListener("click", function () {
      shotEditSelection = {};
      shotEditPasteArmed = false;
      draw();
      shotEditSetHint("Selection cleared.");
    });
    byId("m390ShotUndo").addEventListener("click", shotUndoLastEdit);
    var arrows = bar.querySelectorAll("[data-m390-edit-shift]");
    for (var i = 0; i < arrows.length; i += 1) {
      arrows[i].addEventListener("click", function () { shotShiftSelection(this.getAttribute("data-m390-edit-shift")); });
    }
    return bar;
  }
  ...
  function patchShotEditDrawing() {
    if (window.__mithrilM390ShotEditDrawing) return;
    window.__mithrilM390ShotEditDrawing = true;
    var originalDraw = window.draw;
  ...
  function installShotEditInteraction(canvas) {
    if (!canvas || canvas.getAttribute("data-m390-shot-edit") === "true") return;
    canvas.setAttribute("data-m390-shot-edit", "true");
