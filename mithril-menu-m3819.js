(function () {
  ...
  var RELEASE_VERSION = "m38.19";
  var RELEASE_LABEL = "GPS blank-tag fix, precision quick-fill touch targeting, manual GPS callouts, multi quick fill, and canvas themes";
  var THEME_STORAGE_KEY = "mithrilCanvasThemeV1";
  var THEME_CLASS_PREFIX = "m3819-theme-";
  var THEME_OPTIONS = [
  ...
  function installPrecisionCanvasCoordinates(canvas, type) {
    if (!canvas || canvas.getAttribute("data-m3819-precision-coordinates") === "true") return;
    ...
    canvas.setAttribute("data-m3819-precision-coordinates", "true");
  }

  function enableWheelZoom(canvas) {
    if (!canvas || canvas.getAttribute("data-m3819-wheel-zoom") === "true") return;
    canvas.setAttribute("data-m3819-wheel-zoom", "true");
  ...
  function addShotInfoBackButton() {
    ...
    if (!modal || byId("m3819ShotInfoBack")) return;
    var grid = modal.querySelector(".buttonGrid");
    if (!grid) return;

    var button = document.createElement("button");
    button.id = "m3819ShotInfoBack";
    button.type = "button";
    button.className = "m3819BackMenu";
  ...
  function installShotFrameBridge() {
    ...
    if (!frame || frame.getAttribute("data-m3819-bridge") === "true") return;
    frame.setAttribute("data-m3819-bridge", "true");

    function injectChildScript() {
      try {
        var childDocument = frame.contentDocument;
        if (!childDocument || !childDocument.documentElement) return false;
        if (childDocument.getElementById("mithrilMenuM3818ChildLoader")) return true;

        var script = childDocument.createElement("script");
        script.id = "mithrilMenuM3818ChildLoader";
        script.src = "./mithril-menu-m3819.js?v=38.19-frame";
  ...
  function injectStyles() {
    if (byId("mithrilMenuM3818Styles")) return;

    var style = document.createElement("style");
    style.id = "mithrilMenuM3818Styles";
    style.textContent = [
      ".m3819MenuIntro{margin:0 0 10px;color:#4b4b4b;font-size:13px;font-weight:750;line-height:1.35}",
      ".m3819MenuStack{display:grid;grid-template-columns:1fr;gap:8px}",
      ".m3819MenuStack>button{width:100%;min-height:52px;text-align:left;padding:10px 13px;font-size:16px}",
      ".m3819MenuStack>button.m3819Home{text-align:center}",
      ".m3819Section{display:none;margin-top:9px;padding:10px;border:1px solid #bcbcbc;border-radius:11px;background:#f8f8f8}",
      ".m3819Section.show{display:block}",
      ".m3819SectionTitle{margin:0 0 8px;font-size:16px;font-weight:950}",
      ".m3819SectionHelp{margin:0 0 9px;color:#555;font-size:12px;font-weight:750;line-height:1.35}",
      ".m3819ActionGrid{display:grid;grid-template-columns:1fr 1fr;gap:8px}",
      ".m3819ActionGrid button{min-height:49px}",
      ".m3819ActionGrid .wide{grid-column:1/-1}",
      ".m3819Subpanel{display:none;grid-column:1/-1;padding:9px;border:1px solid #c7c7c7;border-radius:10px;background:white}",
      ".m3819Subpanel.show{display:block}",
      ".m3819DirectionGrid{display:grid;grid-template-columns:1fr 1fr 1fr;grid-template-areas:'. up .' 'left center right' '. down .';gap:8px}",
      ".m3819DirectionGrid button{min-height:50px;padding:7px 5px}",
      ".m3819Up{grid-area:up}.m3819Left{grid-area:left}.m3819Center{grid-area:center}.m3819Right{grid-area:right}.m3819Down{grid-area:down}",
      ".m3819Spacer{visibility:hidden;pointer-events:none}",
      ".m3819DangerZone{margin-top:10px;padding-top:10px;border-top:1px solid #d6aaaa}",
      ".m3819DangerZone button{width:100%;min-height:50px}",
      ".m3819BackMenu{grid-column:1/-1;background:#eef4ff;border-color:#7aa2d8}",
      ".m3819ThemePanel{max-height:46vh;overflow:auto;padding-right:2px}",
      ".m3819ThemeGroupTitle{margin:10px 0 6px;font-size:12px;font-weight:950;color:#555;text-transform:uppercase;letter-spacing:.04em}",
      ".m3819ThemeGrid{display:grid;grid-template-columns:1fr 1fr;gap:8px}",
      ".m3819ThemeButton{min-height:44px;font-size:13px;line-height:1.25;text-align:left}",
      ".m3819ThemeButton.active{background:#1f6feb;color:#fff;border-color:#1f6feb}",
      "html.m3819-theme-gray,body.m3819-theme-gray{background:#2e2e2e !important}",
      "html.m3819-theme-dark-slate,body.m3819-theme-dark-slate{background-color:#232a31 !important;background-image:radial-gradient(circle at 18% 18%, rgba(255,255,255,.06) 0 3px, transparent 4px),radial-gradient(circle at 76% 70%, rgba(0,0,0,.2) 0 18px, transparent 20px),linear-gradient(135deg,#20262d 0%,#313b46 100%) !important;background-size:140px 140px,220px 220px,cover !important;background-attachment:fixed !important}",
      "html.m3819-theme-blue-steel,body.m3819-theme-blue-steel{background-color:#566575 !important;background-image:radial-gradient(circle at 20% 20%, rgba(255,255,255,.08) 0 2px, transparent 3px),linear-gradient(135deg,rgba(255,255,255,.08),rgba(255,255,255,0) 38%),linear-gradient(135deg,#4b5a68 0%,#6b7d8f 100%) !important;background-size:150px 150px,cover,cover !important;background-attachment:fixed !important}",
      "html.m3819-theme-subtle-grid,body.m3819-theme-subtle-grid{background-color:#252e38 !important;background-image:radial-gradient(circle at center, rgba(255,255,255,.03) 1px, transparent 1px),repeating-linear-gradient(0deg, rgba(255,255,255,.06) 0 1px, transparent 1px 26px),repeating-linear-gradient(90deg, rgba(255,255,255,.06) 0 1px, transparent 1px 26px),linear-gradient(135deg,#232b34,#2e3945) !important;background-size:26px 26px,26px 26px,26px 26px,cover !important;background-attachment:fixed !important}",
      "html.m3819-theme-gradient-slate,body.m3819-theme-gradient-slate{background:#54606f !important;background-image:linear-gradient(135deg,#6b7786 0%,#3c4653 100%) !important;background-attachment:fixed !important}",
      "html.m3819-theme-dark-paper,body.m3819-theme-dark-paper{background-color:#35383d !important;background-image:radial-gradient(circle at 25% 25%, rgba(255,255,255,.05) 0 2px, transparent 3px),radial-gradient(circle at 75% 60%, rgba(255,255,255,.03) 0 1px, transparent 2px),linear-gradient(135deg,#2c3035 0%,#44484f 100%) !important;background-size:120px 120px,90px 90px,cover !important;background-attachment:fixed !important}",
      "html.m3819-theme-soft-quarry-tan,body.m3819-theme-soft-quarry-tan{background-color:#b9aea0 !important;background-image:radial-gradient(circle at 20% 20%, rgba(255,255,255,.12) 0 2px, transparent 3px),radial-gradient(circle at 80% 70%, rgba(0,0,0,.08) 0 2px, transparent 3px),linear-gradient(135deg,#c8bdae 0%,#a89b8b 100%) !important;background-size:70px 70px,90px 90px,cover !important;background-attachment:fixed !important}",
      "html.m3819-theme-blast-ember,body.m3819-theme-blast-ember{background-color:#111 !important;background-image:radial-gradient(circle at 15% 78%, rgba(255,110,0,.72) 0 2%, transparent 8%),radial-gradient(circle at 82% 22%, rgba(255,90,0,.48) 0 1.4%, transparent 7%),repeating-linear-gradient(135deg, rgba(255,120,0,.0) 0 18px, rgba(255,110,0,.18) 18px 19px, transparent 19px 34px),linear-gradient(135deg,#090909,#262626) !important;background-attachment:fixed !important}",
      "html.m3819-theme-electric-steel,body.m3819-theme-electric-steel{background-color:#0e2032 !important;background-image:radial-gradient(circle at 50% 60%, rgba(0,150,255,.34) 0 18%, transparent 42%),repeating-linear-gradient(135deg, rgba(255,255,255,.05) 0 2px, transparent 2px 18px),linear-gradient(135deg,#091521,#27425f) !important;background-attachment:fixed !important}",
      "html.m3819-theme-blast-placard,body.m3819-theme-blast-placard{background-color:#111 !important;background-image:linear-gradient(45deg, transparent 38%, rgba(255,150,30,.78) 38% 62%, transparent 62%),linear-gradient(-45deg, transparent 38%, rgba(255,150,30,.78) 38% 62%, transparent 62%),radial-gradient(circle at 80% 25%, rgba(255,165,0,.22) 0 12%, transparent 26%),repeating-linear-gradient(135deg, rgba(255,150,30,.22) 0 12px, transparent 12px 36px),linear-gradient(135deg,#090909,#1b1b1b) !important;background-size:280px 280px,280px 280px,cover,cover,cover !important;background-attachment:fixed !important}",
      "html.m3819-theme-copper-quarry,body.m3819-theme-copper-quarry{background-color:#5a2b11 !important;background-image:radial-gradient(circle at 25% 30%, rgba(255,170,100,.28) 0 14%, transparent 26%),radial-gradient(circle at 72% 68%, rgba(255,210,130,.16) 0 10%, transparent 24%),repeating-linear-gradient(45deg, rgba(255,255,255,.02) 0 12px, rgba(0,0,0,.08) 12px 22px),linear-gradient(135deg,#4c200c,#8a481f) !important;background-attachment:fixed !important}",
      "html.m3819-theme-cobalt-topo,body.m3819-theme-cobalt-topo{background-color:#041c3a !important;background-image:radial-gradient(circle at 18% 72%, transparent 0 40px, rgba(0,180,255,.24) 41px 42px, transparent 43px 54px, rgba(0,180,255,.18) 55px 56px, transparent 57px),radial-gradient(circle at 78% 62%, transparent 0 50px, rgba(0,180,255,.22) 51px 52px, transparent 53px 66px, rgba(0,180,255,.18) 67px 68px, transparent 69px),radial-gradient(circle at 56% 22%, transparent 0 32px, rgba(0,180,255,.18) 33px 34px, transparent 35px 48px, rgba(0,180,255,.15) 49px 50px, transparent 51px),linear-gradient(135deg,#031325,#09447a) !important;background-attachment:fixed !important}",
      "html.m3819-theme-signal-red-slate,body.m3819-theme-signal-red-slate{background-color:#120b0b !important;background-image:radial-gradient(circle at 18% 78%, rgba(255,70,40,.34) 0 18%, transparent 30%),radial-gradient(circle at 84% 20%, rgba(255,60,30,.28) 0 12%, transparent 24%),repeating-linear-gradient(90deg, rgba(255,60,30,.16) 0 2px, transparent 2px 48px),linear-gradient(135deg,#0b0b0b,#272222) !important;background-attachment:fixed !important}",
      "@media(max-width:520px){.m3819QuickButton{font-size:0}.m3819QuickButton:after{content:'Quick';font-size:14px}.m3819FitButton{font-size:0}.m3819FitButton:after{content:'Fit';font-size:14px}.m3819ActionGrid{grid-template-columns:1fr}.m3819ActionGrid .wide{grid-column:auto}.m3819DirectionGrid{grid-template-columns:1fr 1fr 1fr}.m3819ThemeGrid{grid-template-columns:1fr}}"
    ].join("");
  ...
  function updateToolbar(isShot) {
    ...
    for (var i = 0; i < topButtons.length; i += 1) {
      ...
      if (label === "Quick") {
        ...
        topButtons[i].classList.add("m3819QuickButton");
      }
    }
    ...
    if (zoomButtons.length && String(zoomButtons[0].textContent || "").trim() === "Fit") {
      ...
      zoomButtons[0].classList.add("m3819FitButton");
    }
  ...
  function hideAllSections(box, exceptId) {
    var sections = box.querySelectorAll(".m3819Section");
    var buttons = box.querySelectorAll("[data-m3819-section]");
    var i;
    ...
    for (i = 0; i < buttons.length; i += 1) {
      var target = buttons[i].getAttribute("data-m3819-section");
      if (target !== exceptId) setButtonArrow(buttons[i], false);
    }
  }

  function hideSubpanels(section, exceptId) {
    var panels = section.querySelectorAll(".m3819Subpanel");
    var buttons = section.querySelectorAll("[data-m3819-subpanel]");
    var i;
    ...
    for (i = 0; i < buttons.length; i += 1) {
      var target = buttons[i].getAttribute("data-m3819-subpanel");
      if (target !== exceptId) setButtonArrow(buttons[i], false);
    }
  }
  ...
  function wireExpandableSections(box) {
    box.addEventListener("click", function (event) {
      var sectionButton = event.target.closest("[data-m3819-section]");
      if (sectionButton && box.contains(sectionButton)) {
        event.preventDefault();
        var sectionId = sectionButton.getAttribute("data-m3819-section");
      ...
      var subButton = event.target.closest("[data-m3819-subpanel]");
      if (subButton && box.contains(subButton)) {
        event.preventDefault();
        var subId = subButton.getAttribute("data-m3819-subpanel");
        var subpanel = byId(subId);
        if (!subpanel) return;
        var parentSection = subButton.closest(".m3819Section");
  ...
  function resetMenuPanelsWhenClosed(menu) {
    var observer = new MutationObserver(function () {
      ...
      var sections = menu.querySelectorAll(".m3819Section,.m3819Subpanel");
      var buttons = menu.querySelectorAll("[data-m3819-section],[data-m3819-subpanel]");
      var i;
  ...
  function refreshThemeButtons(root) {
    ...
    var buttons = root.querySelectorAll ? root.querySelectorAll('[data-m3819-theme-choice]') : [];
    for (var i = 0; i < buttons.length; i += 1) {
      var button = buttons[i];
      var label = button.getAttribute('data-label') || button.textContent.replace(/^✓\s*/, '').trim();
      button.setAttribute('data-label', label);
      var active = button.getAttribute('data-m3819-theme-choice') === current;
  ...
  function buildThemePickerHtml(panelId) {
    function buildThemeButtons(groupKey) {
      ...
      for (var i = 0; i < THEME_OPTIONS.length; i += 1) {
        ...
        html += '<button type="button" class="m3819ThemeButton" data-m3819-theme-choice="' + THEME_OPTIONS[i].key + '" data-label="' + THEME_OPTIONS[i].label + '">' + THEME_OPTIONS[i].label + '</button>';
      }
      return html;
    }

    return [
      '<button type="button" class="wide" data-m3819-subpanel="' + panelId + '" data-label="Canvas Background" aria-expanded="false">Canvas Background  ›</button>',
      '<div id="' + panelId + '" class="m3819Subpanel">',
      '  <div class="m3819ThemePanel">',
      '    <p class="m3819SectionHelp">Pick a canvas background. The theme applies immediately and stays saved on this device.</p>',
      '    <div class="m3819ThemeGroupTitle">Classic Themes</div>',
      '    <div class="m3819ThemeGrid">' + buildThemeButtons('classic') + '</div>',
      '    <div class="m3819ThemeGroupTitle">Bold Themes</div>',
      '    <div class="m3819ThemeGrid">' + buildThemeButtons('bold') + '</div>',
      '    <div class="m3819ThemeGroupTitle">Default</div>',
      '    <div class="m3819ThemeGrid">' + buildThemeButtons('reset') + '</div>',
      '  </div>',
      '</div>'
    ].join('');
  }
  ...
  function wireThemeButtons(root) {
    ...
    for (var i = 0; i < buttons.length; i += 1) {
      buttons[i].addEventListener('click', function () {
        chooseTheme(this.getAttribute('data-m3819-theme-choice'));
      });
    }
  ...
  function patchDrillMenu() {
    var menu = byId("menuModal");
    if (!menu || menu.getAttribute("data-m3819-patched") === "drill") return;
    var box = menu.querySelector(".box");
    if (!box) return;

    menu.setAttribute("data-m3819-patched", "drill");
    box.innerHTML = [
      '<div class="boxHead"><span>Drill Log Menu</span><button type="button" data-m3819-action="close">Close</button></div>',
      '<p class="m3819MenuIntro">Daily tools stay visible. Setup, exports, and recovery tools open only when needed.</p>',
      '<div class="m3819MenuStack">',
      '  <button type="button" data-m3819-action="info">Drill Log Info</button>',
      '  <button type="button" data-m3819-section="m3819DrillPages" data-label="Page Tools" aria-expanded="false">Page Tools  ›</button>',
      '  <div id="m3819DrillPages" class="m3819Section">',
      '    <div class="m3819SectionTitle">Page Tools</div>',
      '    <div class="m3819ActionGrid">',
      '      <button type="button" class="wide" data-m3819-subpanel="m3819DrillAdd" data-label="Add Page" aria-expanded="false">Add Page  ›</button>',
      '      <div id="m3819DrillAdd" class="m3819Subpanel">',
      '        <p class="m3819SectionHelp">Add a blank page beside the current page.</p>',
      '        <div class="m3819DirectionGrid">',
      '          <button type="button" class="m3819Up" data-m3819-add="up">↑ Add Above</button>',
      '          <button type="button" class="m3819Left" data-m3819-add="left">← Add Left</button>',
      '          <button type="button" class="m3819Center m3819Spacer" tabindex="-1" aria-hidden="true">Current</button>',
      '          <button type="button" class="m3819Right" data-m3819-add="right">Add Right →</button>',
      '          <button type="button" class="m3819Down" data-m3819-add="down">↓ Add Below</button>',
      '        </div>',
      '      </div>',
      '      <button type="button" data-m3819-action="fitAll">Fit All Pages</button>',
      '      <button type="button" class="danger" data-m3819-action="deletePage">Delete Current Page</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m3819-section="m3819DrillExport" data-label="Export & Share" aria-expanded="false">Export &amp; Share  ›</button>',
      '  <div id="m3819DrillExport" class="m3819Section">',
      '    <div class="m3819SectionTitle">Export &amp; Share</div>',
      '    <div class="m3819ActionGrid">',
      '      <button type="button" class="primary wide" data-m3819-action="finish">Finish &amp; Send to Blaster</button>',
      '      <button type="button" data-m3819-action="pdf">Download PDF</button>',
      '      <button type="button" data-m3819-action="csv">Export CSV</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m3819-section="m3819DrillBackup" data-label="Backup & Restore" aria-expanded="false">Backup &amp; Restore  ›</button>',
      '  <div id="m3819DrillBackup" class="m3819Section">',
      '    <div class="m3819SectionTitle">Backup &amp; Restore</div>',
      '    <p class="m3819SectionHelp">Download a recovery copy or restore a previously saved Drill Log.</p>',
      '    <div class="m3819ActionGrid">',
      '      <button type="button" data-m3819-action="backup">Download Backup</button>',
      '      <button type="button" data-m3819-action="restore">Restore Backup</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m3819-section="m3819DrillSettings" data-label="Settings" aria-expanded="false">Settings  ›</button>',
      '  <div id="m3819DrillSettings" class="m3819Section">',
      '    <div class="m3819SectionTitle">Settings</div>',
      '    <div class="m3819ActionGrid">',
      '      <button type="button" class="wide" data-m3819-action="calibrate">Calibrate Employee / Job</button>',
      buildThemePickerHtml("m3819DrillTheme"),
      '      <button id="mithrilUpdateMenuButton" type="button" class="wide" data-m3819-action="updates">Check for Updates</button>',
      '    </div>',
      '    <div class="m3819DangerZone"><button type="button" class="danger" data-m3819-action="clear">Clear Drill Log Data</button></div>',
      '  </div>',
      '  <button id="mithrilHomeMenuButton" type="button" class="m3819Home" data-m3819-action="home">MITHRIL Home</button>',
      '</div>',
    ...
    wireAction(box, '[data-m3819-action="close"]', closeMenu);
    wireAction(box, '[data-m3819-action="info"]', function () { runAndClose("openInfo"); });
    wireAction(box, '[data-m3819-action="fitAll"]', function () { runAndClose("fitAllPages"); });
    wireAction(box, '[data-m3819-action="deletePage"]', function () { runAndClose("deletePage"); });
    wireAction(box, '[data-m3819-action="finish"]', function () { runAndClose("finishAndSendToBlaster"); });
    wireAction(box, '[data-m3819-action="pdf"]', function () { runAndClose("downloadPDF"); });
    wireAction(box, '[data-m3819-action="csv"]', function () { runAndClose("exportCSV"); });
    wireAction(box, '[data-m3819-action="backup"]', function () { runAndClose("downloadJSON"); });
    wireAction(box, '[data-m3819-action="restore"]', function () {
      closeMenu();
      var input = byId("jsonInput");
      if (input) input.click();
    });
    wireAction(box, '[data-m3819-action="calibrate"]', function () { runAndClose("startHeaderCalibration"); });
    wireAction(box, '[data-m3819-action="updates"]', function () { runAndClose("checkUpdatesFromDrillLog"); });
    wireAction(box, '[data-m3819-action="clear"]', function () { callGlobal("clearAll"); });
    wireAction(box, '[data-m3819-action="home"]', function () { runAndClose("returnToSelector"); });
    wireThemeButtons(box);

    var addButtons = box.querySelectorAll("[data-m3819-add]");
    for (var i = 0; i < addButtons.length; i += 1) {
      addButtons[i].addEventListener("click", function () {
        runAndClose("addPageAtDirection", [this.getAttribute("data-m3819-add")]);
      });
    }
  }
  ...
  function patchShotMenu() {
    var menu = byId("menuModal");
    if (!menu || menu.getAttribute("data-m3819-patched") === "shot") return;
    var box = menu.querySelector(".box");
    if (!box) return;

    menu.setAttribute("data-m3819-patched", "shot");
    box.innerHTML = [
      '<div class="boxHead"><span>Shot Diagram Menu</span><button type="button" data-m3819-action="close">Close</button></div>',
      '<p class="m3819MenuIntro">Daily tools stay visible. Page layout, exports, backups, and setup tools open only when needed.</p>',
      '<div class="m3819MenuStack">',
      '  <button type="button" data-m3819-action="info">Shot Info</button>',
      '  <button type="button" data-m3819-section="m3819ShotPages" data-label="Page Tools" aria-expanded="false">Page Tools  ›</button>',
      '  <div id="m3819ShotPages" class="m3819Section">',
      '    <div class="m3819SectionTitle">Page Tools</div>',
      '    <div class="m3819ActionGrid">',
      '      <button type="button" class="wide" data-m3819-subpanel="m3819ShotAdd" data-label="Add Page" aria-expanded="false">Add Page  ›</button>',
      '      <div id="m3819ShotAdd" class="m3819Subpanel">',
      '        <p class="m3819SectionHelp">Add a blank page beside the current page.</p>',
      '        <div class="m3819DirectionGrid">',
      '          <button type="button" class="m3819Up" data-m3819-add="up">↑ Add Above</button>',
      '          <button type="button" class="m3819Left" data-m3819-add="left">← Add Left</button>',
      ...
      '        </div>',
      '      </div>',
      '      <button type="button" class="wide" data-m3819-subpanel="m3819ShotShift" data-label="Shift Hole Data" aria-expanded="false">Shift Hole Data  ›</button>',
      '      <div id="m3819ShotShift" class="m3819Subpanel">',
      '        <p class="m3819SectionHelp">Shift every saved hole entry on the current page. The page itself does not move.</p>',
      '        <div class="m3819DirectionGrid">',
      '          <button type="button" class="m3819Up" data-m3819-shift="up">↑ Shift Up</button>',
      '          <button type="button" class="m3819Left" data-m3819-shift="left">← Shift Left</button>',
      '          <button type="button" class="m3819Center" data-m3819-action="undoShift">Undo</button>',
      '          <button type="button" class="m3819Right" data-m3819-shift="right">Shift Right →</button>',
      '          <button type="button" class="m3819Down" data-m3819-shift="down">↓ Shift Down</button>',
      '        </div>',
      '      </div>',
      '      <button type="button" data-m3819-action="fitAll">Fit All Pages</button>',
      '      <button type="button" class="danger" data-m3819-action="deletePage">Delete Current Page</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m3819-section="m3819ShotExport" data-label="Export & Share" aria-expanded="false">Export &amp; Share  ›</button>',
      '  <div id="m3819ShotExport" class="m3819Section">',
      '    <div class="m3819SectionTitle">Export &amp; Share</div>',
      '    <div class="m3819ActionGrid">',
      '      <button type="button" class="primary wide" data-m3819-action="finish">Finish &amp; Export PDF</button>',
      '      <button type="button" data-m3819-action="shareCsv">Share CSV</button>',
      '      <button type="button" data-m3819-action="csv">Download CSV</button>',
      '      <button type="button" class="wide" data-m3819-action="pdf">Download PDF</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m3819-section="m3819ShotBackup" data-label="Backup & Restore" aria-expanded="false">Backup &amp; Restore  ›</button>',
      '  <div id="m3819ShotBackup" class="m3819Section">',
      '    <div class="m3819SectionTitle">Backup &amp; Restore</div>',
      '    <p class="m3819SectionHelp">Download a recovery copy or restore a previously saved Shot Diagram.</p>',
      '    <div class="m3819ActionGrid">',
      '      <button type="button" data-m3819-action="backup">Download Backup</button>',
      '      <button type="button" data-m3819-action="restore">Restore Backup</button>',
      '    </div>',
      '  </div>',
      '  <button type="button" data-m3819-section="m3819ShotSettings" data-label="Settings" aria-expanded="false">Settings  ›</button>',
      '  <div id="m3819ShotSettings" class="m3819Section">',
      '    <div class="m3819SectionTitle">Settings</div>',
      '    <div class="m3819ActionGrid">',
      '      <button type="button" class="wide" data-m3819-action="calibrate">Field Calibration</button>',
      buildThemePickerHtml("m3819ShotTheme"),
      '      <button id="mithrilUpdateMenuButton" type="button" class="wide" data-m3819-action="updates">Check for Updates</button>',
      '    </div>',
      '    <div class="m3819DangerZone"><button type="button" class="danger" data-m3819-action="clear">Clear Shot Data</button></div>',
      '  </div>',
      '  <button id="mithrilHomeMenuButton" type="button" class="m3819Home" data-m3819-action="home">MITHRIL Home</button>',
      '</div>',
    ...
    wireAction(box, '[data-m3819-action="close"]', closeMenu);
    wireAction(box, '[data-m3819-action="info"]', function () { runAndClose("openShotInfo"); });
    wireAction(box, '[data-m3819-action="fitAll"]', function () { runAndClose("fitAllPages"); });
    wireAction(box, '[data-m3819-action="deletePage"]', function () { runAndClose("deleteCurrentPage"); });
    wireAction(box, '[data-m3819-action="finish"]', function () { runAndClose("finishAndSend"); });
    wireAction(box, '[data-m3819-action="shareCsv"]', function () { runAndClose("emailCSV"); });
    wireAction(box, '[data-m3819-action="csv"]', function () { runAndClose("exportCSV"); });
    wireAction(box, '[data-m3819-action="pdf"]', function () { runAndClose("exportPDFReport"); });
    wireAction(box, '[data-m3819-action="backup"]', function () { runAndClose("downloadJSON"); });
    wireAction(box, '[data-m3819-action="restore"]', function () { runAndClose("triggerLoadJSON"); });
    wireAction(box, '[data-m3819-action="calibrate"]', function () { runAndClose("openFieldCalibration"); });
    wireAction(box, '[data-m3819-action="updates"]', checkShotUpdates);
    wireAction(box, '[data-m3819-action="clear"]', function () { callGlobal("clearAll"); });
    wireAction(box, '[data-m3819-action="home"]', function () { closeMenu(); homeFromShot(); });
    wireAction(box, '[data-m3819-action="undoShift"]', function () { runAndClose("undoLastPageMove"); });
    wireThemeButtons(box);

    var addButtons = box.querySelectorAll("[data-m3819-add]");
    var shiftButtons = box.querySelectorAll("[data-m3819-shift]");
    var i;
    ...
    for (i = 0; i < shiftButtons.length; i += 1) {
      shiftButtons[i].addEventListener("click", function () {
        runAndClose("moveCurrentPageData", [this.getAttribute("data-m3819-shift")]);
      });
    }
  }
  ...
  function injectMultiQuickStyles() {
    if (byId("mithrilMultiQuickM3818Styles")) return;
    var style = document.createElement("style");
    style.id = "mithrilMultiQuickM3818Styles";
    style.textContent = [
      ".m3819QuickIntro{margin:0 0 12px;color:#444;font-size:13px;font-weight:750;line-height:1.4}",
      ".m3819QuickRows{display:grid;gap:10px}",
      ".m3819QuickRow{display:grid;grid-template-columns:78px minmax(130px,1.15fr) minmax(110px,.85fr);gap:8px;align-items:end;padding:9px;border:1px solid #bbb;border-radius:10px;background:#f8f8f8}",
      ".m3819QuickRow.inactive{opacity:.68}",
      ".m3819QuickUse{display:flex;align-items:center;justify-content:center;gap:6px;min-height:46px;padding:6px;border:1px solid #aaa;border-radius:8px;background:#fff;font-size:13px;font-weight:950}",
      ".m3819QuickUse input{width:24px;height:24px;min-height:24px;margin:0;padding:0}",
      ".m3819QuickRow label{min-width:0}",
      ".m3819QuickRow select,.m3819QuickRow input{width:100%;min-height:46px;font-size:17px;padding:8px;border:1px solid #999;border-radius:8px;background:#fff}",
      ".m3819QuickValueCell{min-width:0}",
      ".m3819QuickActions{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:12px}",
      ".m3819QuickActions button{min-height:48px}",
      ".m3819QuickBarSummary{min-width:0;font-size:14px;font-weight:950;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
      ".m3819QuickBarHint{grid-column:1/-1;font-size:12px;font-weight:850;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#333}",
      "#quickBar.m3819MultiQuickBar,#singleFillBar.m3819MultiQuickBar{grid-template-columns:minmax(0,1fr) auto auto!important}",
      "#quickModal .box.m3819QuickModalBox{width:min(650px,96vw)}",
      "#quickModal .box.m3819QuickKeypadOpen{padding-bottom:365px!important}",
      "@media(max-width:520px){.m3819QuickRow{grid-template-columns:68px 1fr}.m3819QuickValueCell{grid-column:2}.m3819QuickActions{grid-template-columns:1fr}.m3819QuickBarSummary{font-size:13px}}"
    ].join("");
    document.head.appendChild(style);
  }
  ...
  function buildMultiQuickRows(prefix, fields) {
    ...
    for (var i = 1; i <= 3; i += 1) {
      html += [
        '<div id="' + prefix + 'Row' + i + '" class="m3819QuickRow">',
        '  <label class="m3819QuickUse"><input id="' + prefix + 'Use' + i + '" type="checkbox" /> Use ' + i + '</label>',
        '  <label>Field<select id="' + prefix + 'Field' + i + '">' + options + '</select></label>',
        '  <label class="m3819QuickValueCell">Value',
  ...
  function installCanvasBackgroundBridge(canvas) {
    if (!canvas || canvas.getAttribute("data-m3819-theme-canvas") === "true") return;
    var context = canvas.getContext && canvas.getContext("2d");
    if (!context || context.__mithrilM3818FillRect) return;
    var originalFillRect = context.fillRect.bind(context);
    context.__mithrilM3818FillRect = originalFillRect;
    ...
    canvas.setAttribute("data-m3819-theme-canvas", "true");
  }
  ...
  function drillQuickPrefix() { return "m3819DrillQuick"; }
  ...
  function updateDrillMultiQuickBar(message) {
    ...
    var summary = byId("m3819DrillQuickSummary");
    var hint = byId("m3819DrillQuickHint");
  ...
  function installDrillQuickPadSupport() {
    if (window.__mithrilM3818DrillPad) return;
    window.__mithrilM3818DrillPad = true;
    ...
    window.showPad = function (id) {
      ...
      if (box && byId("quickModal").classList.contains("show")) box.classList.add("m3819QuickKeypadOpen");
    ...
    window.hidePad = function () {
      ...
      var box = byId("quickModal") && byId("quickModal").querySelector(".box");
      if (box) box.classList.remove("m3819QuickKeypadOpen");
    };
  ...
  function patchDrillMultiQuick() {
    ...
    var bar = byId("quickBar");
    if (!modal || !bar || modal.getAttribute("data-m3819-multi-quick") === "drill") return;
    modal.setAttribute("data-m3819-multi-quick", "drill");
    var box = modal.querySelector(".box");
    if (!box) return;
    box.classList.add("m3819QuickModalBox");
    box.innerHTML = [
      '<div class="boxHead"><span>Quick Fill</span><button type="button" id="m3819DrillQuickClose">Close</button></div>',
      '<p class="m3819QuickIntro">Use up to three different fields. One tap on a hole applies every active row together.</p>',
      '<div class="m3819QuickRows">' + buildMultiQuickRows(drillQuickPrefix(), DRILL_MULTI_QUICK_FIELDS) + '</div>',
      '<div class="m3819QuickActions">',
      '  <button type="button" class="primary" id="m3819DrillQuickOn">Turn On</button>',
      '  <button type="button" id="m3819DrillQuickOff">Turn Off</button>',
      '  <button type="button" class="danger" id="m3819DrillQuickClear">Clear Values</button>',
      '</div>'
    ].join("");

    bar.classList.add("m3819MultiQuickBar");
    bar.innerHTML = [
      '<div id="m3819DrillQuickSummary" class="m3819QuickBarSummary"></div>',
      '<button type="button" id="m3819DrillQuickEdit">Edit</button>',
      '<button type="button" class="danger" id="m3819DrillQuickBarOff">Off</button>',
      '<div id="m3819DrillQuickHint" class="m3819QuickBarHint"></div>'
    ].join("");
    ...
    byId("m3819DrillQuickClose").addEventListener("click", function () { window.closeQuickModal(); });
    byId("m3819DrillQuickOn").addEventListener("click", function () { saveDrillMultiQuick(true); });
    byId("m3819DrillQuickOff").addEventListener("click", function () { saveDrillMultiQuick(false); });
    byId("m3819DrillQuickClear").addEventListener("click", function () {
      for (var i = 1; i <= 3; i += 1) {
    ...
    byId("m3819DrillQuickEdit").addEventListener("click", function () { window.openQuickModal(); });
    byId("m3819DrillQuickBarOff").addEventListener("click", function () { window.turnQuickOff(); });
  ...
  function shotQuickPrefix() { return "m3819ShotQuick"; }
  ...
  function updateShotMultiQuickBar(message) {
    ...
    if (!bar) return;
    bar.classList.toggle("show", !!state.enabled);
    var summary = byId("m3819ShotQuickSummary");
    var hint = byId("m3819ShotQuickHint");
  ...
  function installShotQuickKeypadSupport() {
    if (window.__mithrilM3818ShotPad) return;
    window.__mithrilM3818ShotPad = true;
    ...
    window.scrollActiveLoadInputIntoView = function (id) {
      ...
      if (customIndex(id)) {
        var box = byId("quickModal") && byId("quickModal").querySelector(".box");
        if (box) box.classList.add("m3819QuickKeypadOpen");
      }
    };
  ...
  function patchShotMultiQuick() {
    var modal = byId("quickModal");
    var bar = byId("singleFillBar");
    if (!modal || !bar || modal.getAttribute("data-m3819-multi-quick") === "shot") return;
    modal.setAttribute("data-m3819-multi-quick", "shot");
    var box = modal.querySelector(".box");
    if (!box) return;
    box.classList.add("m3819QuickModalBox");
    box.innerHTML = [
      '<div class="boxHead"><span>Quick Fill</span><button type="button" id="m3819ShotQuickClose">Close</button></div>',
      '<p class="m3819QuickIntro">Use up to three different fields. One tap on a hole applies every active row together.</p>',
      '<div class="m3819QuickRows">' + buildMultiQuickRows(shotQuickPrefix(), SHOT_MULTI_QUICK_FIELDS) + '</div>',
      '<div class="m3819QuickActions">',
      '  <button type="button" class="primary" id="m3819ShotQuickOn">Turn On</button>',
      '  <button type="button" id="m3819ShotQuickOff">Turn Off</button>',
      '  <button type="button" class="danger" id="m3819ShotQuickClear">Clear Values</button>',
      '</div>'
    ].join("");

    bar.classList.add("m3819MultiQuickBar");
    bar.innerHTML = [
      '<div id="m3819ShotQuickSummary" class="m3819QuickBarSummary"></div>',
      '<button type="button" id="m3819ShotQuickEdit">Edit</button>',
      '<button type="button" class="danger" id="m3819ShotQuickBarOff">Off</button>',
      '<div id="m3819ShotQuickHint" class="m3819QuickBarHint"></div>'
    ].join("");
    ...
    byId("m3819ShotQuickClose").addEventListener("click", function () { window.closeQuickEntry(); });
    byId("m3819ShotQuickOn").addEventListener("click", function () { saveShotMultiQuick(true); });
    byId("m3819ShotQuickOff").addEventListener("click", function () { saveShotMultiQuick(false); });
    byId("m3819ShotQuickClear").addEventListener("click", function () {
      for (var i = 1; i <= 3; i += 1) {
        byId(prefix + "Value" + i).value = "";
        byId(prefix + "Bool" + i).value = "Yes";
      }
    ...
    byId("m3819ShotQuickEdit").addEventListener("click", function () { window.openQuickEntry(); });
    byId("m3819ShotQuickBarOff").addEventListener("click", function () { window.quickEnabledOff(); });
  ...
  function injectGPSStyles() {
    if (byId("mithrilGPSM3818Styles")) return;
    var style = document.createElement("style");
    style.id = "mithrilGPSM3818Styles";
    style.textContent = [
      ".m3819GPSDetails{margin-top:12px;border:1px solid #9ab5d6;border-radius:10px;background:#f6faff;overflow:hidden}",
      ".m3819GPSDetails summary{min-height:48px;display:flex;align-items:center;padding:8px 11px;font-size:15px;font-weight:950;cursor:pointer;user-select:none}",
      ".m3819GPSBody{padding:0 10px 10px}",
      ".m3819GPSGrid{display:grid;grid-template-columns:1fr 1fr;gap:9px}",
      ".m3819GPSGrid label{min-width:0}",
      ".m3819GPSGrid input{width:100%;min-height:44px;font-size:17px;padding:8px;border:1px solid #999;border-radius:8px;box-sizing:border-box}",
      ".m3819GPSGrid .wide{grid-column:1/-1}",
      ".m3819GPSActions{display:grid;grid-template-columns:1fr;gap:8px;margin-top:9px}",
      ".m3819GPSActions button{min-height:46px}",
      ".m3819GPSStatus{margin-top:8px;min-height:18px;font-size:12px;font-weight:800;color:#365b82;line-height:1.3}",
      ".m3819GPSMenuGrid{display:grid;grid-template-columns:1fr 1fr;gap:8px}",
      ".m3819GPSMenuGrid button{min-height:48px}",
      ".m3819GPSMenuGrid .wide{grid-column:1/-1}",
      ".m3819GPSArrangeBar{display:none;position:fixed;left:8px;right:8px;bottom:8px;z-index:240;grid-template-columns:minmax(0,1fr) auto auto;gap:8px;align-items:center;background:rgba(255,255,255,.98);border:2px solid #8a4fff;border-radius:12px;padding:8px;box-shadow:0 5px 18px rgba(0,0,0,.38)}",
      ".m3819GPSArrangeBar.show{display:grid}",
      ".m3819GPSArrangeHint{font-size:13px;line-height:1.25;font-weight:900;color:#333;min-width:0}",
      ".m3819GPSArrangeBar button{min-height:44px}",
      "@media(max-width:520px){.m3819GPSGrid{grid-template-columns:1fr}.m3819GPSGrid .wide{grid-column:auto}.m3819GPSActions{grid-template-columns:1fr}.m3819GPSMenuGrid{grid-template-columns:1fr}.m3819GPSMenuGrid .wide{grid-column:auto}.m3819GPSArrangeBar{grid-template-columns:1fr 1fr}.m3819GPSArrangeHint{grid-column:1/-1}}"
    ].join("");
    document.head.appendChild(style);
  }
  ...
  function clampGPSBox(x, y) {
    var maxX = Math.max(0, Number(IMG_W || 0) - GPS_BOX_W - 8);
    var maxY = Math.max(0, Number(IMG_H || 0) - GPS_BOX_H - 8);
    return {
  ...
  function defaultGPSBox(type, holeId) {
    var center = gpsHoleCenter(type, holeId) || { x: GPS_BOX_W, y: GPS_BOX_H };
    var code = 0;
    for (var i = 0; i < String(holeId).length; i += 1) code += String(holeId).charCodeAt(i) * (i + 1);
    var right = code % 2 === 0;
    var below = code % 3 === 0;
    var x = right ? center.x + 58 : center.x - GPS_BOX_W - 58;
    var y = below ? center.y + 48 : center.y - GPS_BOX_H - 48;
    return clampGPSBox(x, y);
  }

  function gpsBoxForRow(type, holeId, row) {
    var x = gpsNumber(row && row.GPSCalloutX);
    var y = gpsNumber(row && row.GPSCalloutY);
    if (x === null || y === null) return Object.assign({ w: GPS_BOX_W, h: GPS_BOX_H }, defaultGPSBox(type, holeId));
    var clamped = clampGPSBox(x, y);
    return { x: clamped.x, y: clamped.y, w: GPS_BOX_W, h: GPS_BOX_H };
  }
  ...
  function drawGPSCallouts(targetCtx, pageNum, type, screenMode) {
    ...
    for (var i = 0; i < ids.length; i += 1) {
      ...
      targetCtx.font = "950 20px Arial";
      targetCtx.fillText("GPS " + holeId, box.x + 10, box.y + 8);
      targetCtx.font = "800 16px Arial";
      targetCtx.fillText(Number(row.GPSLatitude).toFixed(6) + ", " + Number(row.GPSLongitude).toFixed(6), box.x + 10, box.y + 36);
      targetCtx.font = "750 14px Arial";
      targetCtx.fillStyle = "#444";
      targetCtx.fillText("Manual coordinate tag", box.x + 10, box.y + 64);
      if (gpsArrangeMode && gpsArrangeType === type && Number(pageNum) === Number(currentPage)) {
        targetCtx.fillStyle = color;
        targetCtx.fillRect(box.x + box.w - 20, box.y + 6, 12, 12);
      }
      targetCtx.restore();
    }
  }
  ...
  function patchGPSDrawing(type) {
    if (window.__mithrilM3818GPSDrawing) return;
    window.__mithrilM3818GPSDrawing = true;
  ...
  function gpsEditorPrefix(type) {
    return type === "drill" ? "m3819DrillGPS" : "m3819ShotGPS";
  }
  ...
  function installGPSHoleEditor(type) {
    ...
    if (!modal || modal.getAttribute("data-m3819-gps-editor") === type) return;
    ...
    details.className = "m3819GPSDetails";
    details.innerHTML = [
      '<summary>Manual GPS Tag (optional)</summary>',
      '<div class="m3819GPSBody">',
      '  <p class="m3819GPSStatus">Enter coordinates collected with your external GPS equipment. MITHRIL does not request or capture this device\'s location.</p>',
      '  <div class="m3819GPSGrid">',
      ...
      '  </div>',
      '  <div class="m3819GPSActions">',
      '    <button type="button" class="danger" id="' + prefix + 'Clear">Clear GPS Tag</button>',
      '  </div>',
      '  <div id="' + prefix + 'Status" class="m3819GPSStatus">No manual GPS tag saved for this hole.</div>',
      '</div>'
    ].join("");
    ...
    modal.setAttribute("data-m3819-gps-editor", type);
  }
  ...
  function updateGPSMenuState(type) {
    var button = byId(type === "drill" ? "m3819DrillGPSToggle" : "m3819ShotGPSToggle");
    var count = byId(type === "drill" ? "m3819DrillGPSCount" : "m3819ShotGPSCount");
  ...
  function ensureGPSArrangeBar(type) {
    var bar = byId("m3819GPSArrangeBar");
    if (!bar) {
      bar = document.createElement("div");
      bar.id = "m3819GPSArrangeBar";
      bar.className = "m3819GPSArrangeBar";
      bar.innerHTML = [
        '<div id="m3819GPSArrangeHint" class="m3819GPSArrangeHint">Drag GPS boxes to move them. The arrows follow automatically.</div>',
        '<button type="button" id="m3819GPSArrangeReset">Reset Page</button>',
        '<button type="button" class="primary" id="m3819GPSArrangeDone">Done</button>'
      ].join("");
      document.body.appendChild(bar);
      byId("m3819GPSArrangeDone").addEventListener("click", function () { finishGPSArrange(); });
      byId("m3819GPSArrangeReset").addEventListener("click", function () { resetGPSPositions(gpsArrangeType || type); });
    }
    return bar;
  }
  ...
  function startGPSArrange(type) {
    ...
    byId("m3819GPSArrangeHint").textContent = "Page " + currentPage + ": drag GPS boxes to clear important hole data. The arrows follow automatically.";
    if (typeof window.draw === "function") window.draw();
  }
  ...
  function buildGPSMenuHtml(type) {
    var prefix = type === "drill" ? "m3819DrillGPS" : "m3819ShotGPS";
    return [
      '<button type="button" class="wide" data-m3819-subpanel="' + prefix + 'Panel" data-label="GPS Callouts" aria-expanded="false">GPS Callouts  ›</button>',
      '<div id="' + prefix + 'Panel" class="m3819Subpanel">',
      '  <p class="m3819SectionHelp">Coordinates entered manually are stored on each hole. Visible callouts and arrows are also included in PDF exports.</p>',
      '  <div id="' + prefix + 'Count" class="m3819GPSStatus"></div>',
      '  <div class="m3819GPSMenuGrid">',
  ...
  function installGPSMenuTools(type) {
    var settings = byId(type === "drill" ? "m3819DrillSettings" : "m3819ShotSettings");
    if (!settings || settings.getAttribute("data-m3819-gps-menu") === "true") return;
    var grid = settings.querySelector(".m3819ActionGrid");
    if (!grid) return;
    ...
    settings.setAttribute("data-m3819-gps-menu", "true");
    updateGPSMenuState(type);
    if (!window.__mithrilM3818GPSMenuOpenWrapped && typeof window.openMenu === "function") {
      var originalOpenMenuForGPS = window.openMenu;
      window.openMenu = function () {
        updateGPSMenuState(type);
        return originalOpenMenuForGPS.apply(this, arguments);
      };
      window.__mithrilM3818GPSMenuOpenWrapped = true;
    }
  }
  ...
  function hitGPSBox(type, pageNum, localPoint) {
    ...
    for (var i = 0; i < ids.length; i += 1) {
      ...
      if (localPoint.x >= box.x && localPoint.x <= box.x + box.w && localPoint.y >= box.y && localPoint.y <= box.y + box.h) {
  ...
  function moveGPSDrag(eventPoint) {
    ...
    var point = clampGPSBox(local.x - gpsDragState.offsetX, local.y - gpsDragState.offsetY);
  ...
  function installGPSDragging(canvas, type) {
    if (!canvas || canvas.getAttribute("data-m3819-gps-drag") === "true") return;
    ...
    canvas.setAttribute("data-m3819-gps-drag", "true");
  }
