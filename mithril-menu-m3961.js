(function () {
  ...
  var RELEASE_VERSION = "m39.6.1";
  var RELEASE_LABEL = "Physical keyboard entry repair for Shot Diagram and Drill Log, direct number-row and numpad capture, Tab field navigation, Enter Save + Next, keyboard guidance, dual shot volumes, pattern calculations, closable PDF preview, explosive-load extremes, selection tools, GPS callouts, and structured hole conditions";
  ...
  function m396ApplyFieldMode(type) {
    ...
    for (var i = 0; i < ids.length; i += 1) {
      ...
      if (keyboard) {
        ...
        input.removeAttribute("data-temp-readonly");
  ...
  function m396InstallReadonlyRepair(type) {
    ...
    ["focusin", "pointerdown", "mousedown", "click", "beforeinput", "paste"].forEach(function (eventName) {
      box.addEventListener(eventName, function (event) {
        if (!m396KeyboardMode()) return;
        var target = event.target;
        if (!target || m396FieldIds(type).indexOf(String(target.id || "")) < 0) return;
    ...
    if (typeof MutationObserver === "function") {
      var observer = new MutationObserver(function (records) {
        ...
        for (var i = 0; i < records.length; i += 1) {
          ...
          if (target && m396FieldIds(type).indexOf(String(target.id || "")) >= 0 && target.readOnly) {
            m396MakeFieldEditable(type, target);
          }
        }
      });
      observer.observe(box, { subtree: true, attributes: true, attributeFilter: ["readonly", "inputmode", "data-temp-readonly"] });
    }
  }
  ...
  function m396InstallKeyboardHandler(type) {
    ...
    box.addEventListener("keydown", function (event) {
      ...
      if (inSequence) m396MakeFieldEditable(type, target);

      if (inSequence && (event.ctrlKey || event.metaKey) && String(event.key || "").toLowerCase() === "a") {
        event.preventDefault();
        if (typeof target.select === "function") { try { target.select(); } catch (error) {} }
        return;
      }

      if (inSequence && !event.ctrlKey && !event.metaKey && !event.altKey) {
        var token = m396KeyboardToken(type, id, event);
        if (token !== null) {
          event.preventDefault();
          event.stopPropagation();
          m396InsertKeyboardText(target, token);
          return;
        }
        if (event.key === "Backspace" || event.key === "Delete") {
          event.preventDefault();
          event.stopPropagation();
          m396DeleteKeyboardText(target, event.key === "Delete");
          return;
        }
      }
